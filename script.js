(function(){
  ...
  function applyLoadoutSetVisual(element,setId){
    ...
    element.classList.add('loadoutCompleteSet');
    if(set.rarity==='legendary')element.classList.add('loadoutLegendarySet');
  ...
  function playGearEquipEffect(gear,awakened,fullSetActivated){
    let item=gearDefinition(gear),rarity=item&&LOOT_RARITIES[item.rarity],set=item&&item.setId&&SET_BY_ID[item.setId],signature=item&&item.setId&&GEAR_SIGNATURES[item.setId];if(!rarity||!ui.gearCharacterStage)return;
    ui.gearCharacterStage.style.setProperty('--equip-color',signature&&awakened?signature.color:rarity.color);
    ui.gearCharacterStage.classList.remove('equipBurst','equipRank2','equipRank4','signatureAwaken','stormcallerAwaken');
    if(ui.pappaHammerBase)ui.pappaHammerBase.classList.remove('stormcallerAwaken');
    void ui.gearCharacterStage.offsetWidth;
    ui.gearCharacterStage.classList.add('equipBurst','equipRank'+Math.max(2,rarity.rank));
    gearSlotFeedback(item.slot,rarity);
    if(awakened)ui.gearCharacterStage.classList.add('signatureAwaken');
  ...
  function equipGear(uid){
    ...
    let equipping=save.equipped[item.slot]!==uid,beforeTier=item.setId?gearSignatureTier(item.setId):0,beforeFull=equippedFullSetId();
    save.equipped[item.slot]=equipping?uid:null;
    ...
    selectedGearUid=uid;gearTapUid=null;paperDollKey='';paperDollPreviewCache.clear();persist();refreshEquippedGearUI();ui.gearCharacterStage.classList.remove('gearPreviewing');
    if(equipping)playGearEquipEffect(gear,awakened,fullSetActivated);
    let rarity=LOOT_RARITIES[item.rarity];sound(awakened||equipping&&rarity.rank===4?'legendary':equipping&&rarity.rank>=2?'rare':'upgrade');return equipping
  }
  ...
  function renderMobileGearSelection(){
    let gear=selectedGear(),item=gearDefinition(gear);ui.mobileGearSelection.classList.toggle('empty',!item);ui.mobileGearSelectionArt.innerHTML='';ui.mobileGearComparison.innerHTML='';ui.mobileGearSet.innerHTML='';ui.mobileGearEquip.disabled=!item;
  ...
  ui.gearPanel.addEventListener('dragstart',event=>{
    ...
    let geometry=gearDragGeometry(source,event.clientX,event.clientY),dragImage=createGearDragGhost(gear,event.clientX,event.clientY,source,geometry);dragImage.style.top='-1000px';gearDragState={source,uid:source.uid,ghost:null,native:true,geometry};source.element.classList.add('dragging');showGearDropTargets(gear);event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',source.uid);event.dataTransfer.setDragImage(dragImage,geometry.offsetX,geometry.offsetY);setTimeout(()=>dragImage.remove())
  });
