(function(){
  ...
  const {MAX_PLAYER_LEVEL,TOTAL_XP_TO_MAX,MAP_UNLOCK_LEVELS,LOOT_UNLOCK_LEVELS,SET_UNLOCK_LEVELS,xpRequiredForNextLevel:levelXpNeeded,applyXp,sanitizeProgress,xpProgress,playerStatsForLevel,enemyScaleForLevel,bossScaleForLevel,gearScaleForLevel,gearValueScaleForLevel,bossXpReward}=progression;
  const SAVE_KEY='scrapbound_prototype_v1',SAVE_VERSION=12,WORLD={w:2400,h:1600},DEPTH_THRESHOLDS=[0,55,120,195,280],CAMERA_ZOOM={mobile:.72,desktop:.8};
  ...
  function renderGearProgressAndStats(){
    let progress=xpProgress(save.level,save.xp),progressRoot=ui.gearXpText.closest&&ui.gearXpText.closest('.pappaProgress');ui.gearPappaLevel.textContent=progress.level;ui.gearXpFill.style.width=progress.percent*100+'%';ui.gearXpText.textContent=progress.capped?'MAX LEVEL':progress.current+' / '+progress.required+' BOSS XP';if(progressRoot)progressRoot.classList.toggle('maxLevel',progress.capped);
  ...
  function syncXpHud(){let progress=xpProgress(save.level,save.xp),percent=progress.percent*100;ui.xpLevel.textContent=progress.level;ui.xpFill.style.width=percent+'%';ui.xpSpark.style.left=percent+'%';ui.xpText.textContent=progress.capped?'MAX LEVEL':progress.current+' / '+progress.required+' XP';ui.xpHud.classList.toggle('maxLevel',progress.capped)}
  ...
  function grantBossXp(amount){let before=save.level,next=applyXp(save.level,save.xp,amount);save.level=next.level;save.xp=next.xp;syncXpHud();if(next.levelsGained)pulseXpLevel();return next.levelsGained}
  function startBoss(){
  ...
  function showBossLootPath(){
    bossLootPhase='path';let equipped=bossLootResolved.filter(result=>result.action==='equip').length,salvaged=bossLootResolved.length-equipped,nextLevel=progression.clampLevel(save.level+(expeditionCycle+1)*2),nextOdds=bossGearOdds(nextLevel,riskTier+2),best=bossLootRewards[0],rarity=LOOT_RARITIES[gearDefinition(best).rarity];
  ...
  function devAddLevel(){if(save.level>=MAX_PLAYER_LEVEL){devFeedback('MAX LEVEL');return}save.level++;save.xp=0;persist();refreshBase();if(mode==='run'){updateHud();pulseXpLevel()}devFeedback(save.level>=MAX_PLAYER_LEVEL?'MAX LEVEL':'PAPPA LEVEL '+save.level)}
  ...
  function installPlaywrightBridge(){
    ...
    window.__riskTest={
      ...
      setProgress(level,xp){
        ...
        persist();refreshBase();syncXpHud();if(ui.gearOverlay.classList.contains('show'))renderGearProgressAndStats();
        return this.progressionState()
      },
      grantPlayerXp(amount){
        let before=save.level,result=applyXp(save.level,save.xp,amount);save.level=result.level;save.xp=result.xp;
        persist();refreshBase();syncXpHud();if(result.levelsGained&&result.level!==before)pulseXpLevel();
