(function(){
  ...
  const LIMITS={enemies:112,bullets:220,enemyBullets:260,loot:180,particles:420};
  ...
  const HAMMERSTORM={cooldown:8.4,acquireRadius:410,leapDuration:.28,duration:1.08,pulseInterval:.13,baseRadius:108,maxLeap:315};
  ...
  function gearComparisonMarkup(gear){
    let item=gearDefinition(gear);if(!item)return '';let currentMap=loadoutGearMap(null),candidateMap=loadoutGearMap(gear),current=loadoutStats(currentMap),candidate=loadoutStats(candidateMap),worn=currentMap[item.slot],wornItem=gearDefinition(worn),set=item.setId&&SET_BY_ID[item.setId],rows=[
      ...
      ['ATTACK SPEED',current.stats.fire,candidate.stats.fire,'percent'],
  ...
  function renderGearProgressAndStats(){
    ...
    let stats=gearStats();ui.gearStatsSummary.innerHTML='<span data-help-title="Health" data-help="Maximum damage Pappa Hammer can survive before unsecured expedition loot is lost."><small>HEALTH</small><b>'+maxHp()+'</b></span><span data-help-title="Damage" data-help="Base power of each automatic hammer attack before critical hits and relic effects."><small>DAMAGE</small><b>'+shotDamage()+'</b></span><span data-help-title="Reach" data-help="Distance from which coins, caches and nearby pickups are drawn toward Pappa Hammer."><small>REACH</small><b>'+magnetRange()+'</b></span><span data-help-title="Attack Speed" data-help="Faster automatic hammer attacks from equipped gear and active set bonuses."><small>ATTACK</small><b>+'+Math.round(stats.fire*100)+'%</b></span><span data-help-title="Critical Chance" data-help="Chance for an attack to deal amplified critical damage."><small>CRIT</small><b>'+Math.round(stats.crit*100)+'%</b></span>';
  }
  ...
  function tryHammerSpin(){
    ...
    player.spinPack=targets.length;player.spinPeakPack=targets.length;stats=cargoStats();player.spinCd=stats.spinCooldown;player.spinLeap=HAMMERSTORM.leapDuration;player.spinLeapMax=HAMMERSTORM.leapDuration;player.spinStartX=player.x;player.spinStartY=player.y;player.spinTargetX=landing.x;player.spinTargetY=landing.y;player.spinTime=0;player.spinPulse=0;player.spinAngle=0;player.spinHits=0;player.spinKills=0;player.spinCoins=0;player.spinHeal=0;player.spinLifeTargets=new Set();player.inv=Math.max(player.inv,HAMMERSTORM.leapDuration+.14);player.fire=Math.max(player.fire,.3);effects.push({kind:'spinCharge',x:player.x,y:player.y,r:8,maxR:58+Math.min(44,targets.length*1.35),life:.3,max:.3,color:targets.length>=24?'#ffc928':'#d6aa58'});burst(player.x,player.y,targets.length>=24?'#ffc928':'#d6aa58',Math.min(28,14+Math.floor(targets.length/3)),1.1+Math.min(.55,targets.length*.012));shake=Math.max(shake,targets.length>=24?6:4);sound('spinStart');return true
  }
  ...
  function installPlaywrightBridge(){
    ...
    window.__riskTest={
      ...
      hammerstormState(){
        ...
        return{living:living.length,player:{x:Math.round(player.x),y:Math.round(player.y),hp:Math.round(player.hp*100)/100,maxHp:player.maxHp},spin:{cd:Math.round(player.spinCd*100)/100,time:Math.round(player.spinTime*100)/100,leap:Math.round(player.spinLeap*100)/100,hits:player.spinHits,kills:player.spinKills,heal:Math.round(player.spinHeal*100)/100,pack:player.spinPack,damage:Math.round(stats.spinDamage*100)/100,radius:Math.round(stats.spinRadius),duration:Math.round(stats.spinDuration*100)/100},launched,knocked:living.filter(enemy=>enemy.knockTime>0).length,hitStop:Math.round(hitStop*1000)/1000,effects:effects.map(effect=>effect.kind||'ring')}
      },
      startHordeWave(number){
        if(mode!=='run')startRun();postBossDecision=false;routeDecision=false;moduleDecision=false;paused=true;bossActive=false;bossDefeated=false;bossEntity=null;bossLootChest=null;obstacles=[];collisionMap=[];hazards=[];enemyBullets=[];bullets=[];pendingStrikes=[];enemies=[];effects=[];particles=[];caches=[];depth=1;player.x=WORLD.w/2;player.y=WORLD.h/2;player.hp=player.maxHp;player.inv=99;player.fire=99;
