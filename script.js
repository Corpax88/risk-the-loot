(function(){
  ...
  const HAMMERSTORM={acquireRadius:410,leapDuration:.24,pulseInterval:.14,baseRadius:108,maxLeap:315,minSpin:.22};
  const MELEE={reach:96,impactRadius:72,shockwaveRadius:158};
  const HORDE={basePopulation:30,maxPopulation:60,overlapRatio:.44,reinforceAfter:7.8,clearBreather:.58,spawnBatch:4,spawnInterval:.052,cohesionRadius:150};
  const COMMON_ZONES=[
  ...
  const HELP_COPY={
    ...
    spinButton:['Hold Hammerstorm','Hold to dive and keep spinning. Release when you need to reposition. More nearby enemies increase power and reach.'],
  ...
  function updateGearToolbarState(items){
    let sort=GEAR_SORTS.find(entry=>entry.id===gearSort)||GEAR_SORTS[0],slotNames={all:'ALL GEAR',hat:'HELMETS',scarf:'NECK',coat:'CHEST',hammer:'HAMMERS',boots:'BOOTS'},rarity=gearRarityFilter==='all'?'':LOOT_RARITIES[gearRarityFilter].name+' ';ui.gearSortLabel.textContent=sort.name;ui.mobileGearSortLabel.textContent=sort.name;ui.mobileGearFilterLabel.textContent=rarity+(slotNames[gearFilter]||'ALL GEAR');ui.gearInventorySummary.textContent='SHOWING '+items.length+' OF '+inventoryCount()+'  \u00B7  BAG VALUE $'+inventoryValue();ui.gearBagCount.textContent=inventoryCount()+' ITEM'+(inventoryCount()===1?'':'S');renderSellFilteredButton(filteredGearSaleSummary())
  }
  ...
  function tryHammerSpin(){
    ...
    player.spinPack=targets.length;player.spinPeakPack=targets.length;player.spinLeap=targets.length?HAMMERSTORM.leapDuration:.001;player.spinLeapMax=player.spinLeap;player.spinStartX=player.x;player.spinStartY=player.y;player.spinTargetX=landing.x;player.spinTargetY=landing.y;player.spinTime=0;player.spinPulse=0;player.spinAngle=0;player.spinShock=0;player.spinHits=0;player.spinKills=0;player.spinCoins=0;player.spinHeal=0;player.spinLifeTargets=new Set();player.inv=Math.max(player.inv,player.spinLeap+.08);player.fire=Math.max(player.fire,.2);effects.push({kind:'spinCharge',x:player.x,y:player.y,r:8,maxR:58+Math.min(44,targets.length*1.35),life:.26,max:.26,color:targets.length>=24?'#ffc928':'#d6aa58'});burst(player.x,player.y,targets.length>=24?'#ffc928':'#d6aa58',Math.min(28,12+Math.floor(targets.length/3)),1.05+Math.min(.55,targets.length*.012));shake=Math.max(shake,targets.length>=24?6:3);sound('spinStart');return true
  }
  function hammerSpinPulse(stats){
    ...
    for(const enemy of pulseTargets){
      if((enemy.spinHitCd||0)>0)continue;enemy.spinHitCd=Math.max(.085,HAMMERSTORM.pulseInterval/stats.spinRate*.86);let dx=enemy.x-player.x,dy=enemy.y-player.y,length=Math.hypot(dx,dy)||1,nx=dx/length,ny=dy/length,critical=Math.random()<stats.crit,damage=shotDamage()*stats.damage*stats.spinDamage*(enemy.boss?.4:.56)*(critical?1.8:1),force=enemy.boss?70:300+Math.min(330,player.spinPack*9);
    ...
    if(!hits)return;player.spinHits+=hits;player.spinShock=(player.spinShock||0)+1;effects.push({kind:'spinArc',x:player.x,y:player.y,r:radius*.48,maxR:radius,life:.2,max:.2,color:crits?'#f4ead6':'#d6aa58',angle:pulseAngle,hits});burst(player.x,player.y,crits?'#f4ead6':'#d6aa58',Math.min(26,8+hits),1+Math.min(.7,hits*.035));shake=Math.max(shake,Math.min(13,3+hits*.42));if(hits>=4)hitStop=Math.max(hitStop,hits>=14?.055:hits>=8?.038:.022);
    let shockEvery=Math.max(3,6-Math.floor(Math.max(player.spinPack,hits)/15));if(player.spinShock%shockEvery===0)physicalShockwave(player.x,player.y,radius*1.48,shotDamage()*stats.damage*stats.spinDamage*.24,185+Math.min(180,hits*8),'#f0c66a',radius*.72);
  ...
  function resetWaveDirector(delay){waveDirector={number:0,phase:'breather',timer:delay==null?.7:delay,spawnClock:0,queue:[],anchors:[],packId:0,kills:0,startCount:0,targetPopulation:0,clearRewarded:false}}
  ...
  function prepareNextWave(){
    if(bossActive||bossDefeated)return;let living=livingRegularEnemies().length,room=Math.max(0,LIMITS.enemies-living-4);if(room<8){waveDirector.phase='active';waveDirector.timer=.55;return}
  ...
  function rewardWaveClear(){
    if(waveDirector.clearRewarded)return;waveDirector.clearRewarded=true;let large=waveDirector.kills>=18,massacre=waveDirector.kills>=32,bonus=Math.max(3,Math.round((3+depth+waveDirector.number+waveDirector.kills*.08)*lootMultiplier()*.48));runScrap+=bonus;effects.push({kind:'packClear',x:player.x,y:player.y,r:18,maxR:massacre?205:large?158:108,life:massacre?.9:.72,max:massacre?.9:.72,color:massacre?'#ffc928':large?'#d6aa58':'#9eb2d5'});effects.push({kind:'coinText',x:player.x,y:player.y-35,r:0,maxR:0,life:.72,max:.72,color:'#f4ead6',text:'+$'+bonus});burst(player.x,player.y,massacre?'#ffc928':large?'#d6aa58':'#9eb2d5',massacre?46:large?30:18,massacre?2:large?1.5:1.05);shake=Math.max(shake,massacre?11:large?7:3);sound(large?'spinFinish':'pickup')
  }
  function updateWaveDirector(dt){
    ...
    if(waveDirector.phase==='spawning'){
      ...
      if(!waveDirector.queue.length){waveDirector.phase='active';waveDirector.timer=Math.max(4.8,HORDE.reinforceAfter-waveDirector.number*.18)}
      return
    }
    if(waveDirector.phase==='active'){let living=livingRegularEnemies().length;if(!living){rewardWaveClear();waveDirector.phase='breather';waveDirector.timer=HORDE.clearBreather;return}waveDirector.timer-=dt;let overlapAt=Math.max(10,Math.ceil(waveDirector.targetPopulation*HORDE.overlapRatio)),softReinforceAt=Math.ceil(waveDirector.targetPopulation*.7);if(living<=overlapAt||waveDirector.timer<=0&&living<=softReinforceAt)prepareNextWave();else if(waveDirector.timer<=0)waveDirector.timer=.8}
  }
  ...
  function resolveBossLoot(action){
    ...
    setTimeout(()=>{ui.bossLootDecision.classList.remove('resolving','equipped','salvaged');bossLootSelected++;if(bossLootSelected<bossLootRewards.length)renderBossLootDecision();else showBossLootPath()},260)
  }
  function selectBossLootItem(index){if(bossLootPhase!=='decision'||!bossLootRewards.length)return;bossLootSelected=Math.max(0,Math.min(bossLootRewards.length-1,index));renderBossLootDecision()}
  ...
  function updateParticles(dt){let write=0;for(let read=0;read<particles.length;read++){let p=particles[read];p.x+=p.vx*dt;p.y+=p.vy*dt;p.vx*=Math.pow(.06,dt);p.vy*=Math.pow(.06,dt);p.life-=dt;if(p.life>0)particles[write++]=p;else if(particlePool.length<260)particlePool.push(p)}particles.length=write}
  ...
  function physicalShockwave(x,y,radius,damage,force,color,minRadius){
    minRadius=minRadius||0;let hits=0;
    for(const enemy of enemies){if(enemy.dead)continue;let dx=enemy.x-x,dy=enemy.y-y,distance=Math.hypot(dx,dy)||1;if(distance<minRadius||distance>radius+enemy.r||lineBlockedByCover(x,y,enemy.x,enemy.y,2))continue;let falloff=.48+.52*(1-distance/(radius+enemy.r)),nx=dx/distance,ny=dy/distance;enemy.knockVx=nx*force*falloff;enemy.knockVy=ny*force*falloff;enemy.knockTime=enemy.boss?.06:.18;damageEnemy(enemy,playerDamageAgainst(enemy,damage*falloff),x,y,true);hits++}
    effects.push({kind:'pressureWave',x,y,r:Math.max(8,minRadius),maxR:radius,life:.38,max:.38,color:color||'#d6aa58'});if(save.settings.particles)burst(x,y,color||'#d6aa58',Math.min(34,12+hits),1.15+Math.min(.5,hits*.025));if(hits){shake=Math.max(shake,Math.min(10,3+hits*.25));hitStop=Math.max(hitStop,hits>=12?.045:hits>=5?.026:.014)}return hits
  }
  function counterBurst(){physicalShockwave(player.x,player.y,112,shotDamage()*.72,270,'#f4ead6');effects.push({kind:'groundCrack',x:player.x,y:player.y,r:10,maxR:74,life:.34,max:.34,color:'#d6aa58'})}
  ...
  function releaseHammerStrike(strike){
    ...
    let waveRadius=strike.explosive?MELEE.shockwaveRadius:strike.shockwave?MELEE.shockwaveRadius+28:116;physicalShockwave(impactX,impactY,waveRadius,damage*(strike.explosive?.5:.28),strike.explosive?320:220,strike.thermal?'#c83f46':strike.color,MELEE.impactRadius*.7);
  ...
  function fireAt(target){
    ...
    pendingStrikes.push({delay:.12,nx,ny,count,explosive,thermal,banner,starfall,forceCritical,crit:stats.crit,critEcho:stats.critEcho,moonArc:signature.moonbreaker,shockwave,damage:shotDamage()*stats.damage*damageBoost,color:hammerColor});return true
  }
  ...
  function updateEnemyEntity(e,dt){
    ...
    let beforeX=e.x,beforeY=e.y,dx=player.x-e.x,dy=player.y-e.y,l=Math.hypot(dx,dy)||1,nx=dx/l,ny=dy/l,direction=hordeDirection(e,nx,ny,dt);nx=direction.x;ny=direction.y;let arrival=e.spawnGrace>0?.56:1,canAttack=enemyCanAttack(e,ENEMY_VIEW_MARGIN),knocked=e.knockTime>0;e.angle=Math.atan2(dy,dx);
  ...
  function update(dt){
    ...
    if(!spinMotion&&player.dashTime>0){player.dashTime-=dt;player.x+=player.dashX*820*dt;player.y+=player.dashY*820*dt;if(Math.abs(player.dashX)>.08)player.facing=player.dashX<0?-1:1;if(save.settings.particles)Math.random();player.dashFx=(player.dashFx||0)-dt;if(save.settings.particles&&player.dashFx<=0){player.dashFx+=mobileArmory()?.065:.035;spawnParticle({x:player.x-player.dashX*18,y:player.y-player.dashY*18,vx:-player.dashX*40,vy:-player.dashY*40,life:.22,max:.22,r:7,color:stats.stormDash?'#79e7f2':stats.ram?'#f2c14f':'#47c5b6'})}if(stats.ram)for(const e of enemies){if(e.dead||player.ramHits.has(e))continue;let dx=e.x-player.x,dy=e.y-player.y;if(dx*dx+dy*dy<(e.r+player.r+8)*(e.r+player.r+8)){player.ramHits.add(e);damageEnemy(e,stats.ram,player.x,player.y,true)}}}
  ...
  function installPlaywrightBridge(){
    ...
    window.__riskTest={
      ...
      hammerstormState(){
        ...
        return{living:living.length,player:{x:Math.round(player.x),y:Math.round(player.y),hp:Math.round(player.hp*100)/100,maxHp:player.maxHp},spin:{cd:0,time:Math.round(player.spinTime*100)/100,leap:Math.round(player.spinLeap*100)/100,hits:player.spinHits,kills:player.spinKills,heal:Math.round(player.spinHeal*100)/100,pack:player.spinPack,damage:Math.round(stats.spinDamage*100)/100,radius:Math.round(stats.spinRadius),held:spinHeld},playerProjectiles:0,launched,knocked:living.filter(enemy=>enemy.knockTime>0).length,hitStop:Math.round(hitStop*1000)/1000,effects:effects.map(effect=>effect.kind||'ring')}
      },
