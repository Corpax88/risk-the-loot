(function(){
  ...
  const SAVE_KEY='scrapbound_prototype_v1',SAVE_VERSION=14,WORLD={w:2400,h:1600},DEPTH_THRESHOLDS=[0,32,68,108,150],CAMERA_ZOOM={mobile:.72,desktop:.8};
  ...
  // Persistent test fixtures exercise the Legs pipeline without entering live drop tables.
  const FOUNDATION_EQUIPMENT_ITEMS=[
    gearDef('foundationFieldLegs','Foundation Field Trousers','legs','common',12,{hp:4},'#20252d','#9ca6ad',0,{dropDisabled:true,foundationOnly:true}),
    gearDef('foundationGuardLegs','Foundation Guard Trousers','legs','rare',42,{armor:.012},'#16283c','#4da3ff',1,{dropDisabled:true,foundationOnly:true})
  ];
  for(const item of FOUNDATION_EQUIPMENT_ITEMS){item.visual.key=item.id;item.visual.mark=GEAR_SLOT_META.legs.icon;item.visual.asset=null}
  ...
  const LOOT_BY_RARITY=Object.keys(LOOT_RARITIES).reduce((groups,rarity)=>{groups[rarity]=LOOT_ITEMS.filter(item=>item.rarity===rarity&&!item.dropDisabled);return groups},{}),LOOT_BY_ID=Object.fromEntries([...LOOT_ITEMS,...FOUNDATION_EQUIPMENT_ITEMS].map(item=>[item.id,item])),RELIC_POWER_CAP=4;
  const SYNERGIES=[
  ...
  function loadSave(){
    ...
    if(raw.version!==SAVE_VERSION&&raw.version!==13)raw={};
  ...
  function installPlaywrightBridge(){
    ...
    window.__riskTest={
      ...
      spawnFoundationLegs(itemId,equipNow){
        let item=FOUNDATION_EQUIPMENT_ITEMS.find(entry=>entry.id===itemId);if(!item)return null;let gear=rollGearInstance(item,Math.max(item.minLevel||1,save.level),1.08);
        batchEquipmentChanges(()=>{save.gear.push(gear);if(equipNow!==false)save.equipped.legs=gear.uid});persist();refreshBase();if(ui.gearOverlay.classList.contains('show'))renderGearLocker();return{uid:gear.uid,itemId:gear.itemId,equipped:save.equipped.legs===gear.uid}
      },
