(function(){
  ...
  function enterDepth(nextDepth){depth=nextDepth;if(depth===3||depth===5){riskTier++;if(runStats)runStats.risks++}zoneEventTriggered=false;hazards=[];hazardClock=2.2;depthPulse=1.15;fieldRepair(depth===5?.2:.15);triggerFloorSignatures();sound('upgrade');updateZoneHud();updateRouteHud();if(depth===5){startBoss();updateHud();return}let zone=zoneAt(depth),a=Math.random()*Math.PI*2,d=300+Math.random()*180,rareChance=.15+depth*.06+(routeConfig()?routeConfig().relicRare:0);runNotice(zone.name,zone.accent);spawnCache(player.x+Math.cos(a)*d,player.y+Math.sin(a)*d,depth>=3&&Math.random()<rareChance,false)}
  ...
  function destroyEnemy(e){if(e.dead)return;e.dead=true;if(runStats){runStats.kills++;if(e.elite)runStats.elites++}if(e.boss){defeatBoss(e);return}let stats=cargoStats(),signature=stats.signature;if(e.elite&&stats.pursuit){player.dashCd=0;effects.push({x:player.x,y:player.y,r:7,maxR:44,life:.3,max:.3,color:'#d6aa58'})}if(signature.trailwarden){let recovery=e.elite?(signature.trailwarden===2?1.4:.8):(signature.trailwarden===2?.28:.15);player.dashCd=Math.max(0,player.dashCd-recovery)}if(e.elite&&signature.kingsRoad){fieldRepair(signature.kingsRoad===2?.09:.05);if(signature.kingsRoad===2)player.shields=Math.max(player.shields,1)}shake=Math.max(shake,e.elite?6:2.5);let base=1+depth+riskTier+(e.elite?5+depth*2:0)+(e.eventReward||0),coinMult=signature.coinseeker?(signature.coinseeker===2?1.6:1.3):1,eliteBounty=e.elite&&signature.coinseeker===2?4+depth:0,coins=Math.max(1,Math.round(base*lootMultiplier()*coinMult+eliteBounty));runScrap+=coins;effects.push({kind:'coinText',x:e.x,y:e.y,r:0,maxR:0,life:.62,max:.62,color:'#d6aa58',text:'+$'+coins});burst(e.x,e.y,e.elite?'#d6aa58':e.type==='brute'?'#c2b9a5':e.type==='lancer'?'#9eb2d5':'#c83f46',e.elite?20:10,e.elite?1.35:1);sound(e.elite?'rare':'kill');updateHud()}
