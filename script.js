(function(){
  ...
  const LIMITS={enemies:72,bullets:220,enemyBullets:260,loot:180,particles:320};
  const ENEMY_SPAWN_GRACE=.72,ENEMY_VIEW_MARGIN=34,TANK_RUSH_WINDUP=.9,TANK_RUSH_TIME=.42,TANK_RUSH_SPEED=285,TANK_RUSH_RANGE=255;
  const COMMON_ZONES=[
  ...
  const SET_DEFINITIONS=[
    ...
    {id:'riskreaver',name:'RISKREAVER',rarity:'legendary',minLevel:16,color:'#310f18',accent:'#f2c14f',mark:'\u2620',focus:['damage','crit'],bonus:{2:{damage:6},3:{crit:.08},5:{damage:8,fire:.09,crit:.05}}},
  ...
  const GEAR_SIGNATURES={
    ...
    riskreaver:{role:'HIGH RISK',name:'ALL IN',unlock:'At-risk gear and deeper floors raise damage, but also incoming damage.',mastery:'The risk scaling becomes much stronger. The danger does too.',color:'#f2c14f'}
  };
  ...
  function drawPaperDollPattern(layer,item){
    let style=item.visual.style%6,rarity=LOOT_RARITIES[item.rarity],cell=PAPER_DOLL_CELL,scale=cell/128,step=(18-style)*scale;layer.save();layer.globalCompositeOperation='source-atop';layer.strokeStyle=item.visual.accent;layer.fillStyle=item.visual.accent;layer.globalAlpha=.1+rarity.rank*.035;layer.lineWidth=(1.15+rarity.rank*.22)*scale;
    if(style===0){layer.beginPath();layer.moveTo(0,cell*.64);layer.lineTo(cell,cell*.64);layer.stroke()}
    else if(style===1||style===5){for(let x=-cell;x<cell*2;x+=step){layer.beginPath();layer.moveTo(x,cell);layer.lineTo(x+cell,0);layer.stroke()}if(style===5){layer.globalAlpha*=.55;for(let x=0;x<cell*2;x+=step*2){layer.beginPath();layer.moveTo(x,0);layer.lineTo(x-cell,cell);layer.stroke()}}}
    else if(style===2){for(let x=9*scale;x<cell;x+=step){layer.beginPath();layer.moveTo(x,0);layer.lineTo(x,cell);layer.stroke()}}
    else if(style===3){for(let y=10*scale;y<cell;y+=step){layer.beginPath();layer.moveTo(0,y);layer.lineTo(cell,y);layer.stroke()}}
    else{for(let y=10*scale;y<cell;y+=step)for(let x=10*scale;x<cell;x+=step){layer.beginPath();layer.arc(x,y,(rarity.rank>=3?2.1:1.45)*scale,0,Math.PI*2);layer.fill()}}
    if(rarity.rank>=2){layer.globalAlpha=.12+rarity.rank*.035;layer.lineWidth=(1.4+rarity.rank*.25)*scale;for(let x=-cell*.5;x<cell*1.5;x+=cell*.22){layer.beginPath();layer.moveTo(x,cell);layer.lineTo(x+cell*.28,0);layer.stroke()}}
    if(rarity.rank>=3){layer.globalAlpha=.16;for(let y=cell*.16;y<cell;y+=cell*.22)for(let x=cell*.12;x<cell;x+=cell*.24){layer.save();layer.translate(x,y);layer.rotate(Math.PI/4);layer.strokeRect(-2.2*scale,-2.2*scale,4.4*scale,4.4*scale);layer.restore()}}
    if(rarity.rank===4){layer.globalAlpha=.26;layer.strokeStyle='#fff1bf';layer.lineWidth=1.7*scale;layer.beginPath();layer.moveTo(0,cell*.08);layer.lineTo(cell,cell*.08);layer.moveTo(0,cell*.92);layer.lineTo(cell,cell*.92);layer.stroke()}
    layer.restore()
  }
  ...
  function drawPaperDollSilhouette(layer,maskCanvas,slot,item){
    let rarity=LOOT_RARITIES[item.rarity],rank=rarity.rank,radius=2+rank;
    layer.clearRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);layer.save();
    for(let y=-radius-3;y<=radius+3;y+=3)for(let x=-radius-3;x<=radius+3;x+=3)if(x*x+y*y<=(radius+3)*(radius+3))layer.drawImage(maskCanvas,x,y);
    layer.globalCompositeOperation='source-in';layer.fillStyle='#0b1019';layer.fillRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);layer.restore();
    layer.save();for(let y=-radius;y<=radius;y+=2)for(let x=-radius;x<=radius;x+=2)if(x*x+y*y<=radius*radius)layer.drawImage(maskCanvas,x,y);
    layer.globalCompositeOperation='source-in';layer.fillStyle=item.visual.color;layer.globalAlpha=.58;layer.fillRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);layer.restore()
  }
  function drawPaperDollGeometry(layer,slot,item,b,pose,frame){
    if(!b)return;let rarity=LOOT_RARITIES[item.rarity],rank=rarity.rank,x=b.x,y=b.y,w=b.w,h=b.h,accent=item.visual.accent;
    layer.save();layer.lineJoin='round';layer.lineCap='round';
    if(slot==='hat'){
      layer.strokeStyle='#111827';layer.lineWidth=6;layer.beginPath();layer.moveTo(x+w*.27,y+h*.57);layer.quadraticCurveTo(x+w*.5,y+h*.64,x+w*.74,y+h*.57);layer.stroke();layer.strokeStyle=accent;layer.lineWidth=2.5;layer.stroke();drawPaperDollMark(layer,item,x+w*.5,y+h*.42,8+rank)
    }else if(slot==='coat'){
      if(rank>=2)drawPaperDollMark(layer,item,x+w*.39,y+h*.44,9+rank*1.15)
    }else if(slot==='hammer'){
      drawPaperDollMark(layer,item,x+w*.79,y+h*.5,8+rank*1.1)
    }
    layer.restore()
  }
  function composePaperDollPose(pose){
    ...
    for(let frame=0;frame<8;frame++){let sx=frame%4*512,sy=Math.floor(frame/4)*512,mx=frame%4*PAPER_DOLL_MASK_CELL,my=Math.floor(frame/4)*PAPER_DOLL_MASK_CELL,dx=frame%4*PAPER_DOLL_CELL,dy=Math.floor(frame/4)*PAPER_DOLL_CELL;outCtx.drawImage(source,sx,sy,512,512,dx,dy,PAPER_DOLL_CELL,PAPER_DOLL_CELL);for(const slot of PAPER_DOLL_RENDER_ORDER){let item=equippedItem(slot);if(!item)continue;let mask=paperDollMasks[pose][slot],rarity=LOOT_RARITIES[item.rarity];maskLayer.clearRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);maskLayer.globalCompositeOperation='source-over';maskLayer.globalAlpha=1;maskLayer.drawImage(mask,mx,my,PAPER_DOLL_MASK_CELL,PAPER_DOLL_MASK_CELL,0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);let bounds=paperDollMaskBounds(maskLayer);drawPaperDollSilhouette(shapeLayer,maskScratch,slot,item);outCtx.drawImage(shapeScratch,dx,dy);layer.clearRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);layer.globalCompositeOperation='source-over';layer.globalAlpha=1;layer.drawImage(source,sx,sy,512,512,0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);layer.globalCompositeOperation='destination-in';layer.drawImage(maskScratch,0,0);layer.globalCompositeOperation='source-atop';layer.globalAlpha=.24+rarity.rank*.065;layer.fillStyle=item.visual.color;layer.fillRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);layer.globalAlpha=1;drawPaperDollPattern(layer,item);layer.globalCompositeOperation='source-over';outCtx.drawImage(scratch,dx,dy);shapeLayer.clearRect(0,0,PAPER_DOLL_CELL,PAPER_DOLL_CELL);drawPaperDollGeometry(shapeLayer,slot,item,bounds,pose,frame);outCtx.drawImage(shapeScratch,dx,dy)}}return out
  }
  ...
  function activeSetBonuses(){let counts=equippedSetCounts(),active=[];for(const set of SET_DEFINITIONS){let count=counts[set.id]||0;for(const tier of [2,3,5])if(count>=tier&&set.bonus[tier])active.push({set,count,tier,stats:set.bonus[tier]})}return active}
  function gearSignatureTier(setId,counts){let count=(counts||equippedSetCounts())[setId]||0;return count>=5?2:count>=3?1:0}
  ...
  function gearSignatureProfile(){
    let profile={critEcho:0,bulwark:0,stormDash:0,hammerWave:0,riskreaver:0};
    for(const active of activeGearSignatures()){if(active.set.id==='crimsonOath')profile.critEcho=active.tier;else if(active.set.id==='towerBulwark')profile.bulwark=active.tier;else if(active.set.id==='stormrunner')profile.stormDash=active.tier;else if(active.set.id==='hammerChoir')profile.hammerWave=active.tier;else if(active.set.id==='riskreaver')profile.riskreaver=active.tier}
    return profile
  }
  function gearSignatureMarkup(set,count){
    let signature=set&&GEAR_SIGNATURES[set.id];if(!signature)return '';
    let tier=count>=5?2:count>=3?1:0,next=tier===2?'MASTERED':tier===1?'5 PIECES TO MASTER':'3 PIECES TO AWAKEN',effect=tier===2?signature.mastery:signature.unlock;
  ...
  function cargoStats(){
    let over=modulePower('overdrive'),volatile=modulePower('volatile'),mark=modulePower('mark'),plating=platingProtection(modulePower('plating')),gear=gearStats(),signature=gearSignatureProfile(),kinetic=hasSynergy('kinetic'),dashTech=1-schematicLevel('thrusters')*.06,riskCharge=riskTier+Object.keys(lootBag).length,riskDamage=signature.riskreaver?1+Math.min(signature.riskreaver===2?.72:.42,riskCharge*(signature.riskreaver===2?.09:.07)):1,riskTaken=signature.riskreaver&&riskCharge?signature.riskreaver===2?1.18:1.1:1;
    return {burst:Math.min(3,modulePower('burst')),shields:baseShields()+plating.shields,armor:Math.min(.55,plating.armor+gear.armor),speed:1+over*.12+gear.speed,fire:Math.max(.42,(1-over*.11)*(1-gear.fire)),damage:(1+volatile*.34)*riskDamage,hp:Math.max(.58,1-volatile*.11),dashCd:2.2*dashTech*(1-gear.dash),crit:gear.crit,championDamage:1+mark*.1,bossDropChance:Math.min(.28,mark*.07),ram:kinetic?34+shotDamage()*1.2:0,shrapnel:hasSynergy('shrapnel'),counter:hasSynergy('counter'),pursuit:hasSynergy('pursuit'),finisher:hasSynergy('finisher'),critEcho:signature.critEcho,bulwark:signature.bulwark,stormDash:signature.stormDash,hammerWave:signature.hammerWave,riskreaver:signature.riskreaver,riskTaken}
  }
  ...
  function startRun(){
    ...
    player={x:WORLD.w/2,y:WORLD.h/2,r:18,hp:maxHp(),maxHp:maxHp(),speed:235,fire:0,inv:0,angle:0,facing:1,dashCd:0,dashTime:0,dashX:1,dashY:0,lastX:1,lastY:0,shields:baseShields(),guardCd:0,recoil:0,attackAnim:0,attackDuration:.32,animClock:0,volley:0,burstCharge:0,thermalCharges:0,ramHits:new Set()};
    ...
    buildArenaObstacles();buildAdventureDecor();
  ...
  function enterDepth(nextDepth){depth=nextDepth;if(depth===3||depth===5){riskTier++;if(runStats)runStats.risks++}zoneEventTriggered=false;hazards=[];hazardClock=2.2;depthPulse=1.15;fieldRepair(depth===5?.2:.15);sound('upgrade');updateZoneHud();updateRouteHud();if(depth===5){startBoss();updateHud();return}let zone=zoneAt(depth),a=Math.random()*Math.PI*2,d=300+Math.random()*180,rareChance=.15+depth*.06+(routeConfig()?routeConfig().relicRare:0);runNotice(zone.name,zone.accent);spawnCache(player.x+Math.cos(a)*d,player.y+Math.sin(a)*d,depth>=3&&Math.random()<rareChance,false)}
  function tryDash(){if(mode!=='run'||postBossDecision||routeDecision||moduleDecision||paused||!player||player.dashCd>0)return;let move=movement(),dx=move.x||player.lastX||Math.cos(player.angle),dy=move.y||player.lastY||Math.sin(player.angle),l=Math.hypot(dx,dy)||1,stats=cargoStats(),thermal=schematicLevel('thermal'),blast=thermalBlast(thermal);player.dashX=dx/l;player.dashY=dy/l;player.dashTime=.16;player.dashCd=stats.dashCd;player.ramHits=new Set();player.inv=Math.max(player.inv,stats.ram ? .34 : .24);if(thermal)player.thermalCharges=Math.max(player.thermalCharges||0,blast.charges);if(stats.stormDash)releaseStormDash(stats.stormDash,player.dashX,player.dashY);shake=Math.max(shake,3);burst(player.x,player.y,thermal?'#c83f46':stats.stormDash?'#79e7f2':stats.ram?'#d6aa58':'#f4ead6',thermal?16:10,thermal?1.1:.8);sound('dash')}

  function spawnEnemy(elite,forcedType){
    if(enemies.length>=LIMITS.enemies)return;let x=player.x,y=player.y,viewRadius=Math.max(330,Math.hypot(W,H)*.61),minimum=Math.max(350,viewRadius*.84);for(let tries=0;tries<16;tries++){let a=Math.random()*Math.PI*2,d=viewRadius+90+Math.random()*145;x=Math.max(35,Math.min(WORLD.w-35,player.x+Math.cos(a)*d));y=Math.max(35,Math.min(WORLD.h-35,player.y+Math.sin(a)*d));if(Math.hypot(x-player.x,y-player.y)>minimum&&!pointBlocked(x,y,34))break}if(pointBlocked(x,y,34)){let open=openArenaPosition(x,y,34);x=open.x;y=open.y}
    let zone=zoneAt(depth),map=activeMap(),type=forcedType||zone.pool[Math.floor(Math.random()*zone.pool.length)],base=type==='brute'?{r:24,hp:46,speed:50,damage:16}:type==='shooter'?{r:17,hp:23,speed:68,damage:9}:type==='lancer'?{r:16,hp:27,speed:78,damage:14}:{r:15,hp:16,speed:108,damage:10},scale=(1+(depth-1)*.13+riskTier*.1)*map.enemyHp,spawnDuration=ENEMY_SPAWN_GRACE,seed=(x*.013+y*.017+base.r*.11)%(Math.PI*2),entity={x,y,r:base.r*(elite?1.18:1),hp:base.hp*scale*(elite?2.05:1),max:base.hp*scale*(elite?2.05:1),speed:base.speed*(1+depth*.014)*(elite?1.05:1)*map.enemySpeed,damage:base.damage*(elite?1.28:1)*map.enemyDamage,hit:0,attack:0,fire:.8+Math.random(),charge:0,dashTime:0,dashX:0,dashY:0,dashSpeed:0,type,elite:!!elite,angle:0,strafe:Math.random()<.5?-1:1,anim:0,seed,think:.9+Math.abs(Math.sin(seed))*.7,stepFx:.12,recoil:0,aimX:0,aimY:0,spawnGrace:spawnDuration,spawnDuration};enemies.push(entity);return entity
  }
  ...
  function releaseHammerStrike(strike){
    ...
    for(let i=0;i<strike.count&&bullets.length<LIMITS.bullets;i++){
      let spread=(i-(strike.count-1)/2)*.12,cs=Math.cos(spread),sn=Math.sin(spread),vx=strike.nx*cs-strike.ny*sn,vy=strike.nx*sn+strike.ny*cs,critical=Math.random()<strike.crit;
      bullets.push({x:originX+vx*25,y:originY+vy*25,vx:vx*520,vy:vy*520,r:strike.explosive?6:critical?6:5,age:0,life:1.1,damage:strike.damage*(critical?1.8:1),explosive:strike.explosive,critical,critEcho:strike.critEcho,shockwave:strike.shockwave,color:strike.thermal?'#c83f46':strike.shockwave?'#d7c1ff':critical?'#f4ead6':strike.explosive?'#b52f3a':strike.color})
    }
  ...
  function fireAt(target){
    ...
    let count=1+burstVolleyBonus(stats.burst),explosive=thermal||(stats.shrapnel&&player.volley%3===0),damageBoost=thermal?blast.damage:1,shockwave=stats.hammerWave&&player.volley%(stats.hammerWave===2?3:4)===0?stats.hammerWave:0;
    if(thermal)player.thermalCharges--;
    pendingStrikes.push({delay:.12,nx,ny,count,explosive,thermal,crit:stats.crit,critEcho:stats.critEcho,shockwave,damage:shotDamage()*stats.damage*damageBoost,color:hammerColor})
  }
  ...
  function shootEnemy(e,nx,ny){if(e.aimX||e.aimY){nx=e.aimX;ny=e.aimY}let a=Math.atan2(ny,nx),speed=205+depth*5;if(enemyShot(e,a,speed,e.damage,'rivet')){e.recoil=.18;sound('shoot')}else e.fire=Math.max(e.fire,.42)}
  ...
  function destroyEnemy(e){if(e.dead)return;e.dead=true;if(runStats){runStats.kills++;if(e.elite)runStats.elites++}if(e.boss){defeatBoss(e);return}let stats=cargoStats();if(e.elite&&stats.pursuit){player.dashCd=0;effects.push({x:player.x,y:player.y,r:7,maxR:44,life:.3,max:.3,color:'#d6aa58'})}shake=Math.max(shake,e.elite?6:2.5);let base=1+depth+riskTier+(e.elite?5+depth*2:0)+(e.eventReward||0),coins=Math.max(1,Math.round(base*lootMultiplier()));runScrap+=coins;effects.push({kind:'coinText',x:e.x,y:e.y,r:0,maxR:0,life:.62,max:.62,color:'#d6aa58',text:'+$'+coins});burst(e.x,e.y,e.elite?'#d6aa58':e.type==='brute'?'#c2b9a5':e.type==='lancer'?'#9eb2d5':'#c83f46',e.elite?20:10,e.elite?1.35:1);sound(e.elite?'rare':'kill');updateHud()}
  ...
  function startEnemyWindup(e,time,nx,ny){lockEnemyAim(e,nx,ny);e.charge=time}
  function updateEnemyEntity(e,dt){
    e.anim=(e.anim||0)+dt;e.hit=Math.max(0,e.hit-dt);e.attack=Math.max(0,e.attack-dt);e.fire-=dt;e.recoil=Math.max(0,(e.recoil||0)-dt);e.spawnGrace=Math.max(0,(e.spawnGrace||0)-dt);e.think=(e.think||0)-dt;e.stepFx=(e.stepFx||0)-dt;
    let beforeX=e.x,beforeY=e.y,dx=player.x-e.x,dy=player.y-e.y,l=Math.hypot(dx,dy)||1,nx=dx/l,ny=dy/l,arrival=e.spawnGrace>0?.56:1,canAttack=enemyCanAttack(e,ENEMY_VIEW_MARGIN);e.angle=Math.atan2(dy,dx);
    if(e.boss){
      ...
      let approach=l>315?1:l<205?-1:0,orbit=.42+Math.sin(e.anim*1.7+e.seed)*.12,motion=e.charge>0?.32:1;
      ...
      else if(e.fire<=0){if(canAttack){startEnemyWindup(e,.58,nx,ny);e.fire=Math.max(.9,2.25-depth*.05-riskTier*.07)}else e.fire=.2}
    }else if(e.type==='brute'){
      if(e.dashTime>0){e.dashTime-=dt;e.x+=e.dashX*(e.dashSpeed||TANK_RUSH_SPEED)*dt;e.y+=e.dashY*(e.dashSpeed||TANK_RUSH_SPEED)*dt}
      else if(e.charge>0){if(!canAttack)cancelEnemyWindup(e);else{e.charge-=dt;e.x-=e.aimX*e.speed*.13*dt;e.y-=e.aimY*e.speed*.13*dt;if(e.charge<=0){e.charge=0;e.dashTime=TANK_RUSH_TIME;e.dashSpeed=TANK_RUSH_SPEED;e.dashX=e.aimX;e.dashY=e.aimY;e.attack=0;e.recoil=.16;effects.push({kind:'tankImpact',x:e.x,y:e.y,angle:Math.atan2(e.dashY,e.dashX),r:8,maxR:42,life:.24,max:.24,color:'#c83f46'});sound('dash')}}}
      else{let approach=l>118?1:l<82?-.22:0,weave=Math.sin(e.anim*1.8+e.seed)*.11;e.x+=(nx*approach-ny*weave)*e.speed*dt*arrival;e.y+=(ny*approach+nx*weave)*e.speed*dt*arrival;if(e.fire<=0){if(canAttack&&l<TANK_RUSH_RANGE&&l>64){startEnemyWindup(e,TANK_RUSH_WINDUP,nx,ny);e.fire=4.25}else e.fire=.22}}
    }else if(e.type==='lancer'){
      if(e.dashTime>0){e.dashTime-=dt;e.x+=e.dashX*430*dt;e.y+=e.dashY*430*dt}
      else{let approach=l>270?1:l<180?-1:0,orbit=.58+Math.sin(e.anim*2.1+e.seed)*.1;e.x+=(nx*approach-ny*orbit*e.strafe)*e.speed*dt*arrival;e.y+=(ny*approach+nx*orbit*e.strafe)*e.speed*dt*arrival;if(e.charge>0){if(!canAttack)cancelEnemyWindup(e);else{e.charge-=dt;if(e.charge<=0){e.dashTime=.34;e.dashX=e.aimX;e.dashY=e.aimY;effects.push({x:e.x,y:e.y,r:4,maxR:34,life:.2,max:.2,color:'#8f9dff'})}}}else if(e.fire<=0){if(canAttack&&l<360){startEnemyWindup(e,.62,nx,ny);e.fire=2.1-riskTier*.08}else e.fire=.2}}
    }else{
      let weave=Math.sin(e.anim*4.2+e.seed)*.22,pulse=.96+Math.sin(e.anim*5.4+e.seed)*.045;e.x+=(nx-ny*weave)*e.speed*dt*pulse*arrival;e.y+=(ny+nx*weave)*e.speed*dt*pulse*arrival
    }
    let wantedX=e.x,wantedY=e.y;e.x=beforeX;e.y=beforeY;let coverHit=moveAroundCover(e,wantedX-beforeX,wantedY-beforeY);if(coverHit&&e.dashTime>0){e.dashTime=0;e.charge=0;effects.push({kind:'coverImpact',x:e.x,y:e.y,r:4,maxR:24,life:.18,max:.18,color:e.boss?'#ffc928':'#e4e7eb'})}else if(coverHit){moveAroundCover(e,-ny*e.strafe*e.speed*dt*.62,nx*e.strafe*e.speed*dt*.62)}
    ...
    dx=player.x-e.x;dy=player.y-e.y;l=Math.hypot(dx,dy)||1;if(e.spawnGrace<=0&&l<e.r+player.r+3&&e.attack<=0){let rushing=e.type==='brute'&&e.dashTime>0;e.attack=.72;if(rushing){e.dashTime=0;effects.push({kind:'tankImpact',x:e.x,y:e.y,angle:e.angle,r:12,maxR:58,life:.3,max:.3,color:'#c83f46'});shake=Math.max(shake,6)}if(damagePlayer(e.damage*(rushing?.6:1)))return true}return false
  }
  ...
  function update(dt){
    ...
    let move=movement(),stats=cargoStats();player.inv=Math.max(0,player.inv-dt);player.fire-=dt;player.guardCd=Math.max(0,(player.guardCd||0)-dt);player.recoil=Math.max(0,player.recoil-dt);player.attackAnim=Math.max(0,(player.attackAnim||0)-dt);player.animClock=(player.animClock||0)+dt;player.dashCd=Math.max(0,player.dashCd-dt);if(player.dashTime>0){player.dashTime-=dt;player.x+=player.dashX*820*dt;player.y+=player.dashY*820*dt;if(Math.abs(player.dashX)>.08)player.facing=player.dashX<0?-1:1;if(save.settings.particles&&Math.random()<.65)particles.push({x:player.x-player.dashX*18,y:player.y-player.dashY*18,vx:-player.dashX*40,vy:-player.dashY*40,life:.22,max:.22,r:7,color:stats.stormDash?'#79e7f2':stats.ram?'#f2c14f':'#47c5b6'});if(stats.ram)for(const e of enemies){if(e.dead||player.ramHits.has(e))continue;let dx=e.x-player.x,dy=e.y-player.y;if(dx*dx+dy*dy<(e.r+player.r+8)*(e.r+player.r+8)){player.ramHits.add(e);damageEnemy(e,stats.ram,player.x,player.y,true)}}}else{player.x+=move.x*player.speed*stats.speed*dt;player.y+=move.y*player.speed*stats.speed*dt;if(move.x||move.y){player.lastX=move.x;player.lastY=move.y;player.angle=Math.atan2(move.y,move.x);if(Math.abs(move.x)>.08)player.facing=move.x<0?-1:1}}player.x=Math.max(player.r,Math.min(WORLD.w-player.r,player.x));player.y=Math.max(player.r,Math.min(WORLD.h-player.r,player.y));
    let playerWantedX=player.x,playerWantedY=player.y;player.x=playerBeforeX;player.y=playerBeforeY;let playerCoverHit=moveAroundCover(player,playerWantedX-playerBeforeX,playerWantedY-playerBeforeY);if(playerCoverHit&&player.dashTime>0){player.dashTime=0;effects.push({kind:'coverImpact',x:player.x,y:player.y,r:4,maxR:28,life:.2,max:.2,color:'#ffc928'});shake=Math.max(shake,2);sound('hit')}if(updateHazards(dt))return;
    spawnClock-=dt;let interval=Math.max(.66,(2.15-depth*.1-riskTier*.06-zoneProgress()*.32)*activeMap().spawnRate);if(!bossActive&&!bossDefeated&&spawnClock<=0){spawnClock=interval*(.78+Math.random()*.44);spawnEnemy()}
    ...
    for(const b of bullets){b.age=(b.age||0)+dt;b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(projectileHitsCover(b.x,b.y,b.r)){b.dead=true;effects.push({kind:'coverImpact',x:b.x,y:b.y,r:3,maxR:20,life:.16,max:.16,color:b.color||'#ffc928'});continue}for(const e of enemies){if(e.dead||b.dead)continue;let dx=e.x-b.x,dy=e.y-b.y;if(dx*dx+dy*dy<(e.r+b.r)*(e.r+b.r)){b.dead=true;let markBoost=e.elite?stats.championDamage:1,finishBoost=e.boss&&stats.finisher&&e.hp/e.max<=.35?1.2:1;damageEnemy(e,b.damage*markBoost*finishBoost,b.x,b.y,b.explosive||b.critical||b.shockwave);if(b.critical&&b.critEcho)triggerCritEcho(e,b.x,b.y,b.damage,b.critEcho);if(b.shockwave)triggerHammerWave(e,b.x,b.y,b.damage,b.shockwave);if(b.critical)effects.push({x:b.x,y:b.y,r:4,maxR:30,life:.2,max:.2,color:b.critEcho?'#ec9295':'#f4ead6'});if(b.explosive){effects.push({x:b.x,y:b.y,r:8,maxR:74,life:.32,max:.32,color:'#ff8b54'});for(const other of enemies){if(other===e||other.dead)continue;let ox=other.x-b.x,oy=other.y-b.y;if(ox*ox+oy*oy<74*74&&!lineBlockedByCover(b.x,b.y,other.x,other.y,2)){let otherMark=other.elite?stats.championDamage:1,otherFinish=other.boss&&stats.finisher&&other.hp/other.max<=.35?1.2:1;damageEnemy(other,b.damage*.55*otherMark*otherFinish,b.x,b.y,true)}}}sound('hit')}}}bullets=bullets.filter(b=>!b.dead&&b.life>0);
  ...
  function drawPappaHammer(){
    let move=movement(),moving=player.dashTime>0||Math.abs(move.x)+Math.abs(move.y)>.05,attacking=player.attackAnim>0,pose=attacking?'attack':moving?'run':'idle',duration=pose==='attack'?player.attackDuration:pose==='run'?.68:1.65,progress=pose==='attack'?1-player.attackAnim/player.attackDuration:(player.animClock%duration)/duration,frame=Math.max(0,Math.min(7,Math.floor(progress*8))),sprite=paperDollAtlases[pose]||pappaHammerSprites[pose];
    ctx.save();
  ...
  function drawEnemyArt(e){
    let cells={rusher:[0,0],shooter:[1,0],brute:[0,1],lancer:[1,1]},cell=cells[e.type]||cells.rusher,flip=Math.cos(e.angle)<0,box=e.r*(e.type==='brute'?3.05:4.15),spawn=e.spawnDuration?Math.max(0,Math.min(1,1-e.spawnGrace/e.spawnDuration)):1,recoil=Math.min(1,(e.recoil||0)/.18),stride=Math.sin(e.anim*(e.type==='rusher'?8.2:e.type==='lancer'?5.8:e.type==='shooter'?3.8:2.5)+e.seed),bob=e.type==='brute'?Math.abs(stride)*.7:e.type==='shooter'?stride*1.35:Math.abs(stride)*1.8,lean=e.dashTime>0?-.11:e.charge>0?Math.sin(e.anim*15)*.035:e.type==='rusher'?stride*.035:0;
    ctx.save();ctx.globalAlpha=.45+spawn*.55;ctx.translate(-Math.cos(e.angle)*recoil*6,bob-recoil*1.5);ctx.rotate(lean);ctx.scale(.82+spawn*.18,.82+spawn*.18);ctx.fillStyle='rgba(3,7,14,.32)';ctx.beginPath();ctx.ellipse(0,e.r*.72,e.r*(e.type==='brute'?1.15:.88),e.r*.34,0,0,Math.PI*2);ctx.fill();if(e.elite){let markerY=-box*.51-5;ctx.fillStyle='#d6aa58';ctx.strokeStyle='#111827';ctx.lineWidth=1.5;for(let i=-1;i<=1;i++){let x=i*9,y=markerY+Math.abs(i)*2,s=i===0?5:3.5;ctx.beginPath();ctx.moveTo(x,y-s);ctx.lineTo(x+s,y);ctx.lineTo(x,y+s);ctx.lineTo(x-s,y);ctx.closePath();ctx.fill();ctx.stroke()}}if(imageReady(enemyAtlas))drawAtlasCell(enemyAtlas,cell[0],cell[1],2,2,box,box,flip,e.hit>0);else drawEnemyFallback(e);ctx.restore()
  }
  ...
  function drawTankTelegraph(e,p){
    let locked=e.aimX||e.aimY,nx=locked?e.aimX:Math.cos(e.angle),ny=locked?e.aimY:Math.sin(e.angle),angle=Math.atan2(ny,nx),progress=1-Math.min(1,e.charge/TANK_RUSH_WINDUP),start=e.r*.55,length=TANK_RUSH_SPEED*TANK_RUSH_TIME+e.r*1.35,half=e.r+(player?player.r*.78:12),pulse=.72+Math.sin(performance.now()/70)*.16;
  ...
  function drawEnemyAimTelegraph(e,p){
    let locked=e.aimX||e.aimY,nx=locked?e.aimX:Math.cos(e.angle),ny=locked?e.aimY:Math.sin(e.angle),angle=Math.atan2(ny,nx),tyrant=e.boss&&e.bossKind==='tyrant',lagoon=e.boss&&e.bossKind==='leviathan',chargeMax=e.boss?(lagoon?(e.bossStage===1?.82:e.bossStage===2?.92:.62):tyrant?(e.bossStage===1?.78:e.bossStage===2?.72:.55):(e.bossStage===2?.88:e.bossStage===3?.58:.72)):e.type==='lancer'?.62:.58,progress=1-Math.min(1,e.charge/chargeMax),length=e.boss?Math.min(Math.hypot(W,H)*.72,410):e.type==='lancer'?235:Math.min(Math.hypot(W,H)*.62,340),color=lagoon?'121,231,242':tyrant?'200,63,70':e.type==='lancer'?'158,178,213':'214,170,88';ctx.save();ctx.translate(p.x,p.y);ctx.rotate(angle);ctx.strokeStyle='rgba('+color+','+(.28+progress*.66)+')';ctx.lineWidth=e.boss?3:2;ctx.setLineDash(e.type==='lancer'?[13,7]:lagoon?[4,6]:[7,7]);ctx.beginPath();ctx.moveTo(e.r*.5,0);ctx.lineTo(length,0);ctx.stroke();if(lagoon){ctx.globalAlpha=.38+progress*.45;ctx.beginPath();ctx.moveTo(e.r*.6,-7);ctx.quadraticCurveTo(length*.5,7,length,-7);ctx.stroke()}ctx.setLineDash([]);ctx.globalAlpha=1;ctx.fillStyle='rgba('+color+','+(.4+progress*.5)+')';ctx.beginPath();ctx.moveTo(length,0);ctx.lineTo(length-10,-6);ctx.lineTo(length-10,6);ctx.closePath();ctx.fill();ctx.restore()
  }
  ...
  ui.dash.addEventListener('pointerdown',e=>{e.preventDefault();tryDash()});
