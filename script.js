(function(){
  ...
  function paperDollPreview(gear){
    let item=gearDefinition(gear);if(!item||!ui.gearCharacterHero)return;
    let cacheKey=paperDollLoadoutKey()+'>'+gear.uid,cached=paperDollPreviewCache.get(cacheKey);
    if(!cached){
      let resolver=slot=>slot===item.slot?item:equippedItem(slot),fullSetId=equippedFullSetId(resolver),source=paperDollSetReady(fullSetId)?paperDollSetSprites[fullSetId].idle:composePaperDollPose('idle',resolver);
      cached=typeof source.toDataURL==='function'?'url("'+source.toDataURL('image/png')+'")':'url("'+source.src+'")';paperDollPreviewCache.set(cacheKey,cached);
  ...
  function gearSlotFeedback(slot,rarity){
    let slots=ui.gearLoadoutSlots.querySelectorAll?[...ui.gearLoadoutSlots.querySelectorAll('.gearLoadoutSlot[data-slot="'+slot+'"]')]:[];for(const target of slots){target.classList.remove('equipPop');void target.offsetWidth;target.classList.add('equipPop');target.style.setProperty('--gear-color',rarity.color);let burst=document.createElement('span');burst.className='gearSlotParticles';for(let index=0;index<8;index++){let particle=document.createElement('i'),angle=index/8*Math.PI*2,distance=25+(index%3)*8;particle.style.setProperty('--dx',Math.cos(angle)*distance+'px');particle.style.setProperty('--dy',Math.sin(angle)*distance+'px');burst.appendChild(particle)}target.appendChild(burst);setTimeout(()=>{target.classList.remove('equipPop');burst.remove()},620)}
  }
  ...
  function activateGearEntry(event,gear){
    ...
    if(gearPointerType==='touch'&&gearTapUid===gear.uid){equipGear(gear.uid);return}
    selectedGearUid=gear.uid;gearTapUid=gearPointerType==='touch'?gear.uid:null;renderGearLocker();
    if(gearPointerType==='touch'){let anchor=[...ui.gearGrid.querySelectorAll('[data-item]')].find(entry=>entry.dataset.item===gear.uid);if(anchor)showGearHover(gear,anchor,null,true)}
  }
  function renderGearLocker(){
    renderBaseGear();updateGearTurntable();ui.gearPappaLevel.textContent=save.level;let xpGoal=levelXpNeeded(save.level);ui.gearXpFill.style.width=Math.min(100,save.xp/xpGoal*100)+'%';ui.gearXpText.textContent=save.xp+' / '+xpGoal+' BOSS XP';
    ...
    for(const display of displays){
      ...
      if(gear){button.setAttribute('data-item',gear.uid);button.setAttribute('data-info-ready','');button.draggable=true}else{button.setAttribute('data-help-title','Empty '+display.name+' Slot');button.setAttribute('data-help',display.display==='gloves'?'Gloves are visually linked to chest gear. Drop a chest item here to update both areas.':'Drop compatible '+meta.name.toLowerCase()+' gear here, or tap to filter the bag.')}
      button.innerHTML='<i>'+(item?gearArtMarkup(gear,'small'):meta.icon)+'</i><span><small>'+display.name+'</small><b>'+(item?item.name:'EMPTY')+'</b>'+(item?'<em>LEVEL '+gear.level+' \u00B7 EQUIPPED</em>':'')+'</span>';
      button.addEventListener('click',()=>setGearFilter(slot));ui.gearLoadoutSlots.appendChild(button)
    }
    ...
    renderSetSummary();renderGearFilters();renderGearRaritySummary();
    let indexes=new Map(save.gear.map((gear,index)=>[gear.uid,index])),copies=gearCopyCounts(),items=save.gear.filter(gearMatchesFilters).sort(gearSortComparator(indexes));
    ...
    for(const gear of items){
      ...
      entry.className='gearBagSlot rarity'+rarity.rank+(equipped?' equipped':'')+(selected?' selected':'');entry.style.setProperty('--gear-color',rarity.color);entry.style.setProperty('--gear-glow',rarity.glow);entry.setAttribute('data-item',gear.uid);entry.setAttribute('data-info-ready','');entry.setAttribute('role','option');entry.setAttribute('aria-selected',selected?'true':'false');entry.draggable=true;
      ...
      entry.addEventListener('click',event=>activateGearEntry(event,gear));entry.addEventListener('dblclick',event=>{event.preventDefault();equipGear(gear.uid)});entry.addEventListener('contextmenu',event=>{event.preventDefault();equipGear(gear.uid)});ui.gearGrid.appendChild(entry)
    }
    renderGearDetail(save.gear.find(gear=>gear.uid===selectedGearUid),copies);let summary=filteredGearSaleSummary(),sort=GEAR_SORTS.find(entry=>entry.id===gearSort)||GEAR_SORTS[0];ui.gearEmpty.classList.toggle('show',items.length===0);ui.gearBagCount.textContent=inventoryCount()+' ITEM'+(inventoryCount()===1?'':'S');ui.gearSortLabel.textContent=sort.name;ui.gearInventorySummary.textContent='SHOWING '+items.length+' OF '+inventoryCount()+'  \u00B7  BAG VALUE $'+inventoryValue();renderSellFilteredButton(summary)
  }
  ...
  ui.gearPanel.addEventListener('pointerdown',event=>{
    gearPointerType=event.pointerType||'mouse';let source=gearDragSource(event.target);if(!source||event.button&&event.button!==0)return;
