(function(){
  ...
  function gearSignatureProfileFromCounts(counts){
    ...
    profile.critEcho=profile.crimsonOath;profile.bulwark=profile.towerBulwark;profile.stormDash=profile.stormrunner;profile.hammerWave=profile.hammerChoir;profile.gravityWell=profile.blackHole;profile.handlava=profile.lavaSet;
    return profile
  }
  ...
  function gearArtMarkup(gear,size){
    let item=gearDefinition(gear);if(!item)return '';
    let visual=item.visual,rarity=LOOT_RARITIES[item.rarity],stormcaller=Number.isInteger(visual.stormcallerColumn),blackHole=!stormcaller&&Number.isInteger(visual.blackHoleColumn),lava=!stormcaller&&!blackHole&&Number.isInteger(visual.lavaColumn),dedicated=stormcaller||blackHole||lava,legendary=!dedicated&&Number.isInteger(visual.legendaryRow)&&Number.isInteger(visual.legendaryColumn),setSprite=!dedicated&&!legendary&&Number.isInteger(visual.setGearRow)&&Number.isInteger(visual.setGearColumn),set=item.setId&&SET_BY_ID[item.setId],classes=['gearArt',item.slot];
  ...
  function awardEnemyXp(enemy){
    if(!enemy)return null;if(enemy.xpAwarded){xpTelemetry.duplicateSkips++;return null}enemy.xpAwarded=true;if(enemy.noXp||enemy.cancelled)return null;let kind=enemy.boss?'boss':enemy.elite?'elite':'enemy',amount=getEnemyXpReward({enemyType:enemy.type,enemyLevel:enemy.level||save.level,elite:!!enemy.elite,boss:!!enemy.boss,depth:expeditionFloor(),difficulty:riskTier});return applyPlayerXpReward(amount,kind)
  }
  function startBoss(){
  ...
  function separateEnemyHorde(dt){
    let cellSize=64,buckets=new Map(),regular=enemies.filter(enemy=>!enemy.dead&&!enemy.boss&&!enemy.handlavaHeld&&(enemy.knockTime||0)<=0);
  ...
  function handlavaChooseLanding(arm,target,stats){
    ...
    for(let ring=radius;ring>=58&&!found;ring-=16)for(let index=0;index<12;index++){let step=index===0?0:Math.ceil(index/2)*(index%2?1:-1),testAngle=angle+step*Math.PI/12,x=Math.max(target.r,Math.min(WORLD.w-target.r,player.x+Math.cos(testAngle)*ring)),y=Math.max(target.r,Math.min(WORLD.h-target.r,player.y+Math.sin(testAngle)*ring));if(!pointBlocked(x,y,target.r+5)){landingX=x;landingY=y;found=true;break}}arm.throwX=landingX;arm.throwY=landingY
  }
  ...
  function updateHandlava(dt,stats){
    ...
    for(const arm of state.arms){
      ...
      else if(arm.phase==='swing'){let target=arm.target,fromX=target.x,fromY=target.y,eased=.5-Math.cos(progress*Math.PI)*.5,radius=arm.swingRadius+(Math.min(HANDLAVA.followUpMax,stats.spinRadius*.82)-arm.swingRadius)*eased,angle=arm.swingAngle+arm.side*Math.PI*1.34*eased;target.x=Math.max(target.r,Math.min(WORLD.w-target.r,player.x+Math.cos(angle)*radius));target.y=Math.max(target.r,Math.min(WORLD.h-target.r,player.y+Math.sin(angle)*radius));target.angle=angle+arm.side*Math.PI*.5;arm.x=target.x;arm.y=target.y;handlavaSwingCollisions(arm,state,stats,fromX,fromY);if(progress>=1)handlavaBeginThrow(arm,stats)}
  ...
  function update(dt){
    ...
    updateWaveDirector(dt);updateHandlava(dt,stats);
  ...
  function drawAdventureLootSprite(item,r){
    let visual=item.visual,stormcaller=Number.isInteger(visual.stormcallerColumn),blackHole=!stormcaller&&Number.isInteger(visual.blackHoleColumn),lava=!stormcaller&&!blackHole&&Number.isInteger(visual.lavaColumn),dedicated=stormcaller||blackHole||lava,legendary=!dedicated&&Number.isInteger(visual.legendaryRow)&&Number.isInteger(visual.legendaryColumn),setSprite=!dedicated&&!legendary&&Number.isInteger(visual.setGearRow)&&Number.isInteger(visual.setGearColumn),atlas=stormcaller?stormcallerDropAtlas:blackHole?blackHoleDropAtlas:lava?lavaDropAtlas:legendary?legendaryDropAtlas:setSprite?setGearDropAtlas:gearDropAtlas,cell=dedicated?80:legendary?LEGENDARY_DROP_CELL:setSprite?SET_GEAR_DROP_CELL:GEAR_DROP_CELL;
    if(!imageReady(atlas))return false;
    let column=stormcaller?visual.stormcallerColumn:blackHole?visual.blackHoleColumn:lava?visual.lavaColumn:legendary?visual.legendaryColumn:setSprite?visual.setGearColumn:visual.atlasColumn,row=dedicated?0:legendary?visual.legendaryRow:setSprite?visual.setGearRow:visual.atlasRow,size=r*(stormcaller?4.45:blackHole?4.3:lava?4.35:legendary?4.15:setSprite?3.85:3.65);
