(function(){
  ...
  const paperDollSetSprites={
    ...
    natureSet:{
      idle:loadImage('assets/nature-hammer-idle-v1.png'),
      run:loadImage('assets/nature-hammer-run-v1.png'),
      attack:loadImage('assets/nature-hammer-attack-v1.png')
    }
  };
  ...
  let player,enemies=[],pendingStrikes=[],enemyBullets=[],lootDrops=[],particles=[],effects=[],hazards=[],decor=[],obstacles=[],collisionMap=[],collisionSpatial=new Map(),collisionQueryStamp=0,caches=[],cargo=[],lootBag={},guildTerrain=null,currentMapSeed=0,guildSpawnCursor=0,worldStreamer=null,activeWorldRegion=null,activatedWorldRegions=new Set();
  ...
  function selectedBulkGear(){return save.gear.filter(gear=>gearBulkSelection.has(gear.uid)&&!gearIsEquipped(gear))}
  ...
  function resetFilteredSaleArm(){sellFilterArmedKey='';sellFilterArmedUntil=0;gearBulkConfirmAction='';gearBulkConfirmUntil=0}
  function clearGearBulkSelection(){gearBulkMode=false;gearBulkSelection.clear();resetFilteredSaleArm()}
  function renderGearBulkActions(){
    let summary=bulkGearSummary(),confirming=performance.now()<=gearBulkConfirmUntil?gearBulkConfirmAction:'';ui.selectGearItems.classList.toggle('active',gearBulkMode);ui.selectGearLabel.textContent=gearBulkMode?'CANCEL SELECT':'SELECT ITEMS';ui.selectGearSummary.textContent=summary.count+' SELECTED';
    ui.sellFilteredGear.disabled=!summary.count;ui.sellFilteredGear.classList.toggle('armed',confirming==='sell');ui.sellFilteredLabel.textContent=confirming==='sell'?'CONFIRM SELL':'SELL ITEMS';ui.sellFilteredSummary.textContent=summary.count?summary.count+' ITEM'+(summary.count===1?'':'S')+'  \u00B7  $'+summary.value:'SELECT FIRST';
    ui.salvageSelectedGear.disabled=!summary.count;ui.salvageSelectedGear.classList.toggle('armed',confirming==='salvage');ui.salvageSelectedLabel.textContent=confirming==='salvage'?'CONFIRM SALVAGE':'SALVAGE ITEMS';ui.salvageSelectedSummary.textContent=summary.count?summary.materials+' MATERIAL'+(summary.materials===1?'':'S')+(summary.cores?'  + '+summary.cores+' CORE':''):'SELECT FIRST'
  }
  function toggleGearBulkMode(){if(gearBulkMode)clearGearBulkSelection();else{gearBulkMode=true;resetFilteredSaleArm();notice('SELECT UNEQUIPPED ITEMS','#d6aa58')}updateGearInventoryStates();renderGearBulkActions()}
  function toggleBulkGear(uid){let gear=save.gear.find(entry=>entry.uid===uid);if(!gear)return;if(gearIsEquipped(gear)){notice('EQUIPPED GEAR IS PROTECTED','#79a67e');return}if(gearBulkSelection.has(uid))gearBulkSelection.delete(uid);else gearBulkSelection.add(uid);selectedGearUid=uid;resetFilteredSaleArm();updateGearInventoryStates();renderGearSelection();renderGearBulkActions()}
  ...
  function activateGearEntry(event,gear){
    ...
    if(gearBulkMode){toggleBulkGear(gear.uid);return}
  ...
  function updateGearInventoryStates(){
    gearPerf.inventoryStateUpdates++;for(const [uid,entry] of gearItemNodes){let gear=save.gear.find(item=>item.uid===uid);if(!gear)continue;let equipped=gearIsEquipped(gear),selected=uid===selectedGearUid,bulkSelected=gearBulkSelection.has(uid);entry.classList.toggle('equipped',equipped);entry.classList.toggle('selected',selected&&!gearBulkMode);entry.classList.toggle('bulkSelectable',gearBulkMode&&!equipped);entry.classList.toggle('bulkSelected',bulkSelected);entry.setAttribute('aria-selected',gearBulkMode?bulkSelected?'true':'false':selected?'true':'false');entry.draggable=!mobileArmory()&&!gearBulkMode;let art=entry.querySelector('.gearBagArt'),mark=art&&art.querySelector('.gearWornMark'),bulk=art&&art.querySelector('.gearBulkMark');if(equipped&&!mark&&art){mark=document.createElement('i');mark.className='gearWornMark';mark.innerHTML='&#10003;';art.appendChild(mark)}else if(!equipped&&mark)mark.remove();if(bulkSelected&&!bulk&&art){bulk=document.createElement('i');bulk.className='gearBulkMark';bulk.innerHTML='&#10003;';art.appendChild(bulk)}else if(!bulkSelected&&bulk)bulk.remove()}
  }
  function renderGearInventoryArea(){
    ...
    for(const gear of items){
      ...
      let bulkSelected=gearBulkSelection.has(gear.uid);entry.className='gearBagSlot rarity'+rarity.rank+(equipped?' equipped':'')+(selected&&!gearBulkMode?' selected':'')+(gearBulkMode&&!equipped?' bulkSelectable':'')+(bulkSelected?' bulkSelected':'');entry.style.setProperty('--gear-color',rarity.color);entry.style.setProperty('--gear-glow',rarity.glow);entry.setAttribute('data-item',gear.uid);entry.setAttribute('data-info-ready','');entry.setAttribute('role','option');entry.setAttribute('aria-selected',gearBulkMode?bulkSelected?'true':'false':selected?'true':'false');entry.draggable=!mobileArmory()&&!gearBulkMode;
      entry.innerHTML='<span class="gearBagArt">'+gearArtMarkup(gear,'bag')+(copyCount>1?'<b class="gearCopyCount">x'+copyCount+'</b>':'')+(equipped?'<i class="gearWornMark">&#10003;</i>':'')+(bulkSelected?'<i class="gearBulkMark">&#10003;</i>':'')+(item.setId?'<i class="gearSetMark" title="'+escapeMarkup(SET_BY_ID[item.setId].name)+' set">'+escapeMarkup(SET_BY_ID[item.setId].mark)+'</i>':'')+'</span><span class="gearBagSlotMeta"><b>LV '+gear.level+'</b><small>'+Math.round(gearScore(gear))+' PWR</small></span><em class="gearBagSlotName">'+item.name+'</em>';
      entry.addEventListener('click',event=>activateGearEntry(event,gear));entry.addEventListener('dblclick',event=>{if(mobileArmory()||gearBulkMode)return;event.preventDefault();equipGear(gear.uid)});entry.addEventListener('contextmenu',event=>{event.preventDefault();if(!mobileArmory()&&!gearBulkMode)equipGear(gear.uid)});ui.gearGrid.appendChild(entry);gearItemNodes.set(gear.uid,entry)
    }
  ...
  function drawMiniMap(){
    ...
    let cacheCount=visibleCaches.length+(bossLootChest&&!bossLootChest.cleanupLocked?1:0);ui.miniMapCacheCount.textContent=cacheCount?'\u2726 '+cacheCount:'';ui.miniMapCacheCount.classList.toggle('show',cacheCount>0)
  }
  function draw(){
