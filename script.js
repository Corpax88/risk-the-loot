(function(){
  ...
  function applyPaperDollAtlases(next,key){paperDollAtlases=next;paperDollKey=key;paperDollPreviewUrl=next.idle&&typeof next.idle.toDataURL==='function'?'url("'+next.idle.toDataURL('image/png')+'")':'url("'+next.idle.src+'")';ui.pappaHammerBaseSprite.style.backgroundImage=paperDollPreviewUrl;ui.gearCharacterHero.style.backgroundImage=paperDollPreviewUrl;ui.gearCharacterHero.dataset.gearVisualKey=paperDollLoadoutKey()}
  ...
  function restorePaperDollPreview(){
    if(!ui.gearCharacterHero)return;if(paperDollPreviewUrl&&ui.gearCharacterHero.style.backgroundImage!==paperDollPreviewUrl)ui.gearCharacterHero.style.backgroundImage=paperDollPreviewUrl;ui.gearCharacterStage.classList.remove('gearPreviewing','gearLegendaryPreview')
  }
  ...
  function tryHammerSpin(spinStats){
    ...
    player.spinPack=targets.length;player.spinPeakPack=targets.length;player.spinLeap=targets.length?HAMMERSTORM.leapDuration:.001;player.spinLeapMax=player.spinLeap;player.spinAutoRemaining=0;player.spinAutoDuration=0;player.spinManual=false;player.spinStartX=player.x;player.spinStartY=player.y;player.spinTargetX=landing.x;player.spinTargetY=landing.y;player.spinTime=0;player.spinPulse=0;player.spinAngle=0;player.spinShock=0;player.spinHits=0;player.spinKills=0;player.spinCoins=0;player.spinHeal=0;if(player.spinLifeTargets)player.spinLifeTargets.clear();else player.spinLifeTargets=new Set();player.spinVortexX=player.x;player.spinVortexY=player.y;player.blackHoleAge=0;player.blackHolePulse=0;player.blackHoleTrail=0;player.inv=Math.max(player.inv,player.spinLeap+.08);player.fire=Math.max(player.fire,.2);player.spinFinishing=false;player.spinFinishingUntil=0;spinInputState.starts++;spinInputState.activeInstances=1;spinInputState.maxActiveInstances=Math.max(spinInputState.maxActiveInstances,spinInputState.activeInstances);
