(function(){
  ...
  const SET_DEFINITIONS=[
    ...
    {id:'stormrunner',name:'STORMCALLER',rarity:'legendary',dropBand:'epic',statScale:1.28,valueBase:92,minLevel:4,color:'#1e4e63',accent:'#9ed9e5',mark:'\u21AF',focus:['speed','dash'],bonus:{2:{speed:.045},3:{dash:.06},5:{fire:.055,speed:.04}}},
    ...
    {id:'crimsonOath',name:'CRIMSON OATH',rarity:'legendary',dropBand:'elevated',statScale:1.55,valueBase:150,minLevel:7,color:'#631e2c',accent:'#ec9295',mark:'\u2605',focus:['damage','crit'],bonus:{2:{damage:3.8},3:{crit:.055},5:{damage:5.2,fire:.06}}},
    {id:'moonbreaker',name:'MOONBREAKER',rarity:'legendary',dropBand:'elevated',statScale:1.55,valueBase:150,minLevel:8,color:'#24274f',accent:'#aebcf0',mark:'\u263D',focus:['crit','fire'],bonus:{2:{crit:.045},3:{fire:.065},5:{crit:.07,damage:3.5}}},
    {id:'kingsRoad',name:"KING'S ROAD",rarity:'legendary',dropBand:'elevated',statScale:1.55,valueBase:150,minLevel:9,color:'#152d55',accent:'#d6aa58',mark:'\u265B',focus:['hp','loot'],bonus:{2:{hp:34},3:{loot:.09},5:{armor:.06,damage:3.2}}},
    {id:'phantomCourt',name:'PHANTOM COURT',rarity:'legendary',dropBand:'elevated',statScale:1.55,valueBase:150,minLevel:10,color:'#36314d',accent:'#b7c7d9',mark:'\u25C9',focus:['speed','armor'],bonus:{2:{speed:.065},3:{armor:.055},5:{dash:.08,crit:.05}}},
    {id:'starforge',name:'STARFORGE',rarity:'legendary',dropBand:'elevated',statScale:1.55,valueBase:150,minLevel:11,color:'#30243c',accent:'#f0b83e',mark:'\u2737',focus:['damage','armor'],bonus:{2:{damage:4.2},3:{armor:.05},5:{damage:5,crit:.045}}},
    {id:'grandVoyager',name:'GRAND VOYAGER',rarity:'legendary',dropBand:'elevated',statScale:1.55,valueBase:150,minLevel:12,color:'#234253',accent:'#d6c58f',mark:'\u2295',focus:['loot','speed'],bonus:{2:{loot:.08},3:{speed:.07},5:{magnet:30,fire:.055}}},
    {id:'riskreaver',name:'RISKREAVER',rarity:'legendary',dropBand:'apex',minLevel:16,color:'#310f18',accent:'#f2c14f',mark:'\u2620',focus:['damage','crit'],tiers:[2,4,5],signaturePieces:4,bonus:{2:{speed:.08},4:{damage:6,crit:.05},5:{damage:8,fire:.08,crit:.05}}},
    {id:'grandVault',name:'GRAND VAULT',rarity:'legendary',dropBand:'apex',minLevel:18,color:'#eee5d2',accent:'#d6aa58',mark:'\u25C8',focus:['hp','armor'],bonus:{2:{hp:55},3:{armor:.08},5:{loot:.14,hp:45,damage:4}}},
    {id:'crownlessKing',name:'CROWNLESS KING',rarity:'legendary',dropBand:'apex',minLevel:20,color:'#151d33',accent:'#f0c66a',mark:'\u2654',focus:['damage','hp'],bonus:{2:{damage:5,hp:28},3:{crit:.07,armor:.05},5:{speed:.08,fire:.08,loot:.1}}},
    {id:'fatebound',name:'FATEBOUND',rarity:'legendary',dropBand:'apex',minLevel:24,color:'#20203c',accent:'#e65a62',mark:'\u221E',focus:['crit','speed'],bonus:{2:{crit:.07,speed:.05},3:{fire:.085,dash:.09},5:{damage:9,crit:.06,loot:.08}}},
    {id:'blackHole',name:'BLACK HOLE',rarity:'legendary',dropBand:'apex',minLevel:22,color:'#07152d',accent:'#4bbcff',mark:'\u2299',focus:['damage','armor'],bonus:{2:{armor:.05,damage:4},3:{crit:.06,fire:.06},5:{damage:8,fire:.07,crit:.04}}}
  ];
  ...
  const gearItemNodes=new Map(),gearPerf={fullRenders:0,gridRenders:0,slotRenders:0,incrementalRenders:0,baseRenders:0,detailRenders:0,mobileDetailRenders:0,inventoryStateUpdates:0,setRenders:0,rarityRenders:0,paperDollBuilds:0,persistWrites:0};
  ...
  function restorePaperDollPreview(){
    if(!ui.gearCharacterHero)return;let atlas=paperDollAtlases.idle;if(atlas){let image=typeof atlas.toDataURL==='function'?'url("'+atlas.toDataURL('image/png')+'")':'url("'+atlas.src+'")';ui.gearCharacterHero.style.backgroundImage=image}ui.gearCharacterStage.classList.remove('gearPreviewing','gearLegendaryPreview')
  }
  ...
  function gearArtMarkup(gear,size){
    let item=gearDefinition(gear);if(!item)return '';
    let visual=item.visual,rarity=LOOT_RARITIES[item.rarity],stormcaller=Number.isInteger(visual.stormcallerColumn),blackHole=!stormcaller&&Number.isInteger(visual.blackHoleColumn),dedicated=stormcaller||blackHole,legendary=!dedicated&&Number.isInteger(visual.legendaryRow)&&Number.isInteger(visual.legendaryColumn),setSprite=!dedicated&&!legendary&&Number.isInteger(visual.setGearRow)&&Number.isInteger(visual.setGearColumn),set=item.setId&&SET_BY_ID[item.setId],classes=['gearArt',item.slot];
  ...
  function bossGearOdds(level,nextRisk,chosenMap,chosenRoute){level=Math.max(1,Math.floor(level||save.level));let tier=nextRisk==null?riskTier:nextRisk,map=chosenMap||activeMap(),path=chosenRoute===undefined?routeConfig():chosenRoute,routeRare=path?path.lootRare:0,mapRare=map.rarityBonus||0,apex=level>=16?Math.min(.24,.012+(level-16)*.007+tier*.005+routeRare*.2+mapRare*.38):0,elevated=level>=8?Math.min(.48,.08+(level-8)*.014+tier*.01+routeRare*.55+mapRare*.72):0,epic=level>=4?Math.min(.62,.2+(level-4)*.018+tier*.012+routeRare*.45+mapRare):0;epic=Math.min(epic,Math.max(0,.92-apex-elevated));return {rare:Math.max(.08,1-apex-elevated-epic),epic,legendary:apex+elevated,apex,elevated,high:apex+elevated+epic}}
  ...
  function vaultDropBand(){let level=save.level,cycle=save.vaultCycle,apex=level>=16?Math.min(.2,.035+cycle*.012+(level-16)*.006):0,elevated=level>=8?Math.min(.46,.16+cycle*.018+(level-8)*.012):0,epic=level>=4?Math.min(.72,.48+cycle*.015+(level-4)*.012):0,roll=Math.random();return roll<apex?'apex':roll<apex+elevated?'elevated':roll<apex+elevated+epic?'epic':'rare'}
  function rollVaultGear(){let gear=rollBossGear(Math.max(1,save.level),vaultDropBand()),def=gearDefinition(gear);gear.quality=Math.min(1.35,Math.max(1.02,gear.quality+.08));let scale=(1+(gear.level-1)*.055)*gear.quality,stats={};for(const key of Object.keys(def.stats||{}))stats[key]=roundGearStat(key,def.stats[key]*scale*(.97+Math.random()*.1));gear.stats=stats;gear.value=Math.max(gear.value,Math.round(def.value*(1+(gear.level-1)*.08)*gear.quality*1.18));return gear}
  ...
  function setView(next){if(next!==mode)cancelSkillGesture('scene transition');mode=next;ui.base.classList.toggle('active',next==='base');ui.expedition.classList.toggle('active',next==='run');ui.game.classList.toggle('runMode',next==='run');setMusicMode(next);if(next==='run')requestAnimationFrame(()=>{resize();last=performance.now()})}
  ...
  function gearComparisonData(gear){
    let item=gearDefinition(gear);if(!item)return null;let currentMap=loadoutGearMap(null),candidateMap=loadoutGearMap(gear),current=loadoutStats(currentMap),candidate=loadoutStats(candidateMap),worn=currentMap[item.slot],wornItem=gearDefinition(worn),damageBase=8+(save.level-1)*.3+save.weapon*1.45;
    let values={
  ...
  function hideGearHover(uid){if(uid&&hoverGearUid!==uid)return;hoverGearUid=null;restorePaperDollPreview();gearHoverPreview.classList.remove('show','touchPreview','legendaryPreview');gearHoverPreview.style.bottom='';gearHoverPreview.setAttribute('aria-hidden','true')}
  ...
  function equipGear(uid){
    ...
    let equipping=save.equipped[item.slot]!==uid,record=beginEquipPerf(gear,item,equipping),visualPreview=equipPerfStep(record,'preview',()=>paperDollPreviewImage(gear)),beforeTier=item.setId?gearSignatureTier(item.setId):0,beforeFull=equippedFullSetId();
    equipPerfStep(record,'state',()=>{save.equipped[item.slot]=equipping?uid:null;selectedGearUid=uid;gearTapUid=null;paperDollKey='';paperDollPreviewCache.clear()});equipPerfMark(record,'state');
    ...
    equipPerfStep(record,'saveQueue',()=>scheduleEquipPersist(record));equipPerfMark(record,'saveQueued');refreshEquippedGearUI(record,visualPreview);ui.gearCharacterStage.classList.remove('gearPreviewing','gearLegendaryPreview');
    equipPerfStep(record,'feedback',()=>{if(equipping)playGearEquipEffect(gear,awakened,fullSetActivated)});
    let rarity=LOOT_RARITIES[item.rarity],activeVisualSet=equippedFullSetId();record.visualSetId=activeVisualSet||null;record.usesProductionSkin=paperDollSetReady(activeVisualSet);equipPerfStep(record,'sound',()=>sound(awakened||equipping&&rarity.rank===4?'legendary':equipping&&rarity.rank>=2?'rare':'upgrade'));finishEquipPerf(record);return equipping
  }
  ...
  function refreshArmoryCharacter(previewImage){
    gearPerf.baseRenders++;let equipped=GEAR_SLOTS.map(equippedGear).filter(Boolean),highest=equipped.slice().sort((a,b)=>{let ad=gearDefinition(a),bd=gearDefinition(b);return LOOT_RARITIES[bd.rarity].rank-LOOT_RARITIES[ad.rarity].rank||gearScore(b)-gearScore(a)})[0],highestDef=gearDefinition(highest),sets=equippedSetCounts(),bestSet=Object.keys(sets).sort((a,b)=>sets[b]-sets[a])[0],fullSetId=equippedFullSetId(),profile=equippedRarityProfile();
    applyLoadoutRarity(ui.gearCharacterStage,profile);applyLoadoutSetVisual(ui.gearCharacterStage,fullSetId);ui.gearLoadoutName.textContent=equipped.length+'/5 EQUIPPED'+(bestSet&&sets[bestSet]>=2?' \u00B7 '+SET_BY_ID[bestSet].name:highestDef?' \u00B7 '+LOOT_RARITIES[highestDef.rarity].name:'');ui.gearEquippedCount.textContent=equipped.length+' / 5';ui.gearPreviewName.textContent=highestDef?highestDef.name.toUpperCase():'FIELD LOADOUT';ui.gearPreviewName.style.setProperty('--loadout-color',profile.color);if(previewImage)ui.gearCharacterHero.style.backgroundImage=previewImage;ui.gearCharacterHero.dataset.gearVisualKey=paperDollLoadoutKey()
  }
  function renderGearDetail(gear,copies){
    ...
    let rarity=LOOT_RARITIES[item.rarity],equipped=gearIsEquipped(gear),worn=equippedGear(item.slot),set=item.setId&&SET_BY_ID[item.setId],score=Math.round(gearScore(gear)*10)/10,wornScore=worn?Math.round(gearScore(worn)*10)/10:0,delta=Math.round((score-wornScore)*10)/10,comparison=equipped?'EQUIPPED':!worn?'OPEN SLOT':delta>0?'UPGRADE  +'+delta:delta<0?'LOWER  '+delta:'SIDEGRADE',root=document.createElement('article');
  ...
  function activateGearEntry(event,gear){
    ...
    selectedGearUid=gear.uid;gearTapUid=touch?gear.uid:null;paperDollPreview(gear);updateGearInventoryStates();renderGearSelection()
  }
  ...
  function renderGearInventoryArea(){
    ...
    for(const gear of items){
      ...
      entry.innerHTML='<span class="gearBagArt">'+gearArtMarkup(gear,'bag')+(copyCount>1?'<b class="gearCopyCount">x'+copyCount+'</b>':'')+(equipped?'<i class="gearWornMark">&#10003;</i>':'')+(item.setId?'<i class="gearSetMark" title="'+escapeMarkup(SET_BY_ID[item.setId].name)+' set">'+escapeMarkup(SET_BY_ID[item.setId].mark)+'</i>':'')+'</span><span class="gearBagSlotMeta"><b>LV '+gear.level+'</b><small>'+Math.round(gearScore(gear))+' PWR</small></span><em class="gearBagSlotName">'+item.name+'</em>';
  ...
  function refreshEquippedGearUI(record,previewImage){
    gearPerf.incrementalRenders++;equipPerfStep(record,'character',()=>refreshArmoryCharacter(previewImage));equipPerfMark(record,'character');equipPerfStep(record,'turntable',updateGearTurntable);equipPerfStep(record,'stats',renderGearProgressAndStats);equipPerfMark(record,'stats');equipPerfStep(record,'slots',renderGearLoadoutSlots);equipPerfMark(record,'slot');equipPerfStep(record,'sets',renderSetSummary);equipPerfStep(record,'cards',updateGearInventoryStates);equipPerfMark(record,'card');equipPerfStep(record,'comparison',renderGearSelection);equipPerfMark(record,'comparison')
  }
  ...
  function closeGearLocker(){flushEquipPersist();clearGearDropState();hideGearHover();closeGearMobileSheet();ui.gearOverlay.classList.remove('show');refreshBase()}
  ...
  function activateSpinControl(){
    ...
    spinHeld=true;return tryHammerSpin()
  }
  ...
  function beginSpinGesture(event){
    if(skillGesture.pointerId!=null||usesLightningDash(cargoStats()))return false;
  ...
  function tryHammerSpin(){
    if(mode!=='run'||postBossDecision||routeDecision||moduleDecision||paused||!player||usesLightningDash(cargoStats())||player.spinTime>0||player.spinLeap>0||player.dashTime>0)return false;
  ...
  function startBoss(){
    ...
    bossActive=true;extracting=0;hazards=[];ui.extractOverlay.classList.remove('show');enemies=[];clearEnemyProjectiles();
  ...
  function salvageReward(gear){let item=gearDefinition(gear);return item&&(SALVAGE_REWARDS[item.dropBand]||SALVAGE_REWARDS[item.rarity])||SALVAGE_REWARDS.common}
  ...
  function showBossLootRitual(rewards,def,headline){
    cancelSkillGesture('loot decision');let sorted=rewards.slice().sort((a,b)=>{let ad=gearDefinition(a),bd=gearDefinition(b);return LOOT_RARITIES[bd.rarity].rank-LOOT_RARITIES[ad.rarity].rank||gearScore(b)-gearScore(a)});bossLootRewards=sorted;bossLootSelected=0;bossLootResolved=[];postBossDecision=true;ui.bossLootGrid.innerHTML='';ui.bossLootOverlay.classList.remove('show');renderBossLootDecision();void ui.bossLootOverlay.offsetWidth;ui.bossLootOverlay.classList.add('show');ui.lootToast.classList.remove('show','legendary')
  }
  ...
  function burst(x,y,color,count,power){if(!save.settings.particles)return;color=noticeTone(color);let sourceCount=mobileArmory()?Math.max(3,Math.ceil(count*.68)):count,renderCount=perfState.active==='high'?sourceCount:Math.max(2,Math.ceil(sourceCount*qualityProfile().particleScale));renderCount=Math.min(renderCount,particleLimit()-particles.length);let iterations=perfState.active==='high'?renderCount:sourceCount;for(let i=0;i<iterations;i++){let a=Math.random()*Math.PI*2,s=(20+Math.random()*80)*(power||1),life=.25+Math.random()*.35,r=1.5+Math.random()*3;if(i<renderCount)spawnParticle({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life,max:.6,r,color})}}
  ...
  function destroyEnemy(e){
    ...
    burst(e.x,e.y,lightningKill?'#79e7f2':e.elite?'#d6aa58':e.type==='brute'?'#c2b9a5':e.type==='lancer'?'#9eb2d5':'#c83f46',e.elite?20:launchKill?18:10,e.elite?1.35:launchKill?1.45:1);if(!launchKill)sound(e.elite?'rare':'kill');updateHud()
  }
  ...
  function update(dt){
    ...
    if(spinMotion!=='leap'){let playerWantedX=player.x,playerWantedY=player.y;player.x=playerBeforeX;player.y=playerBeforeY;let playerCoverHit=moveAroundCover(player,playerWantedX-playerBeforeX,playerWantedY-playerBeforeY);if(playerCoverHit&&player.dashTime>0){player.dashTime=0;effects.push({kind:'coverImpact',x:player.x,y:player.y,r:4,maxR:28,life:.2,max:.2,color:'#ffc928'});shake=Math.max(shake,2);sound('hit')}}updateStormcallerMovementFx(dt,Math.hypot(player.x-playerBeforeX,player.y-playerBeforeY),stats);if(updateHazards(dt))return;
  ...
  function draw(){
    ...
    let effectShadow=qualityProfile().shadowBlur;for(const fx of effects){
      ...
      else if(fx.kind==='hitSpark'){ctx.rotate(fx.angle||0);ctx.strokeStyle=fx.color;ctx.lineCap='round';ctx.shadowColor=fx.color;ctx.shadowBlur=5*effectShadow;ctx.lineWidth=Math.max(1,3.5*(1-progress));for(let ray=0;ray<6;ray++){ctx.rotate(Math.PI/3);ctx.beginPath();ctx.moveTo(radius*.18,0);ctx.lineTo(radius,0);ctx.stroke()}}
      ...
      else if(fx.kind==='spinArc'){ctx.rotate((fx.angle||0)+progress*1.8);ctx.strokeStyle=fx.color;ctx.shadowColor=fx.color;ctx.shadowBlur=10*effectShadow;ctx.lineCap='round';ctx.lineWidth=Math.max(2,11*(1-progress));for(let arm=0;arm<3;arm++){ctx.rotate(Math.PI*2/3);ctx.beginPath();ctx.arc(0,0,radius,-.62,.62);ctx.stroke()}}
      else if(fx.kind==='spinCrit'){ctx.rotate(progress*Math.PI);ctx.strokeStyle=fx.color;ctx.shadowColor=fx.color;ctx.shadowBlur=12*effectShadow;ctx.lineWidth=Math.max(1,5*(1-progress));for(let arm=0;arm<4;arm++){ctx.rotate(Math.PI/2);ctx.beginPath();ctx.moveTo(5,0);ctx.lineTo(radius,0);ctx.stroke()}}
  ...
  function installPlaywrightBridge(){
    ...
    window.__riskTest={
      ...
      spawnHammerstormPack(count,options){
        ...
        player.x=WORLD.w/2;player.y=WORLD.h/2;player.hp=options.hurt?player.maxHp*.5:player.maxHp;player.inv=99;player.fire=99;player.spinCd=0;player.spinTime=0;player.spinLeap=0;player.spinAutoRemaining=0;player.spinAutoDuration=0;player.spinManual=false;player.spinHits=0;player.spinKills=0;player.spinCoins=0;player.spinHeal=0;player.spinLifeTargets=new Set();player.lightningPhase='idle';player.lightningTime=0;player.lightningRate=0;player.lightningQueue=0;player.lightningTarget=null;player.lightningHistoryTargets=Array(LIGHTNING_DASH.targetHistorySize).fill(null);player.lightningHistoryTimes=Array(LIGHTNING_DASH.targetHistorySize).fill(0);player.lightningHistoryCursor=0;player.lightningBossSide=1;player.lightningImpacts=0;player.lightningPresses=0;player.lightningChainHits=0;player.lightningKills=0;player.lightningTempo=0;player.lightningLastTap=-99;player.stormAwakening=0;spinHeld=false;
        if(options.fullRiskreaver||options.fullBlackHole||options.fullLightning){
          let set=SET_BY_ID[options.fullBlackHole?'blackHole':options.fullLightning?'stormrunner':'riskreaver'];
      ...
      equipmentInventory(){
        return save.gear.map(gear=>{let item=gearDefinition(gear);return{uid:gear.uid,itemId:gear.itemId,name:item&&item.name,slot:item&&item.slot,rarity:item&&item.rarity,setId:item&&item.setId||null,equipped:gearIsEquipped(gear),power:Math.round(gearScore(gear)*10)/10}})
      },
      equipmentState(){
  ...
  window.addEventListener('pointerup',endGearPointerDrag,true);window.addEventListener('pointercancel',endGearPointerDrag,true);window.addEventListener('pagehide',()=>{flushEquipPersist();clearGearDropState()});
  ...
  ui.spin.addEventListener('pointerdown',event=>{
    event.preventDefault();event.stopPropagation();let lightning=usesLightningDash(cargoStats()),accepted=activateSpinControl();
