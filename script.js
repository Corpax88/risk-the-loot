(function(){
  ...
  function refreshPappaHammerAssetAtlases(){
    if(!imageReady(pappaHammerImage)||!PAPER_DOLL_POSES.every(pose=>ASSET_GEAR_SLOTS.every(slot=>imageReady(paperDollMasks[pose][slot]))))return false;let sourceBounds=imageAlphaBounds(pappaHammerImage,0,0,pappaHammerImage.naturalWidth,pappaHammerImage.naturalHeight);if(!sourceBounds)return false;
  ...
  const stormcallerWeaponlessFigure=loadImage('assets/equipment/stormcaller/legendary_stormcaller_weaponless_figure_01.png?v=20260805-anatomy-v2');
  ...
  const ASSET_GEAR_SLOTS=['hat','scarf','coat','hammer','boots'];
  const SET_GEAR_SLOTS=['hat','scarf','chest','weapon','boots'];
  const GEAR_SLOTS=['hat','cape','chest','legs','boots','scarf','weapon','necklace','ring1','ring2'];
  const VISUAL_GEAR_SLOTS=['cape','legs','boots','chest','scarf','hat','weapon'];
  const CONTENT_TO_EQUIPMENT_SLOT=Object.freeze({hat:'hat',scarf:'scarf',coat:'chest',hammer:'weapon',boots:'boots',cape:'cape',chest:'chest',legs:'legs',weapon:'weapon',necklace:'necklace',ring1:'ring1',ring2:'ring2'});
  const EQUIPMENT_TO_ASSET_SLOT=Object.freeze({hat:'hat',cape:'coat',chest:'coat',legs:'legs',scarf:'scarf',weapon:'hammer',boots:'boots'});
  const GEAR_SLOT_META={
    hat:{name:'HAT',icon:'\u25B2',visual:true},cape:{name:'CAPE',icon:'\u25C1',visual:true},chest:{name:'CHEST',icon:'\u25A5',visual:true},legs:{name:'LEGS',icon:'\u2551',visual:true},boots:{name:'BOOTS',icon:'\u21A5',visual:true},scarf:{name:'SCARF',icon:'\u2248',visual:true},weapon:{name:'WEAPON',icon:'\u2692',visual:true},
    necklace:{name:'NECKLACE',icon:'\u25C7',visual:false},ring1:{name:'RING 1',icon:'\u25CB',visual:false},ring2:{name:'RING 2',icon:'\u25CB',visual:false}
  };
  const EQUIPMENT_SLOT_ANCHORS=Object.freeze({
    cape:Object.freeze({x:0,y:0,scale:1}),legs:Object.freeze({x:0,y:0,scale:1}),boots:Object.freeze({x:0,y:0,scale:1}),chest:Object.freeze({x:0,y:0,scale:1}),scarf:Object.freeze({x:0,y:0,scale:1}),hat:Object.freeze({x:0,y:0,scale:1}),weapon:Object.freeze({x:0,y:0,scale:1})
  });
  function equipmentSlot(slot){return CONTENT_TO_EQUIPMENT_SLOT[slot]||slot}
  function assetSlot(itemOrSlot){if(itemOrSlot&&typeof itemOrSlot==='object')return itemOrSlot.assetSlot||EQUIPMENT_TO_ASSET_SLOT[itemOrSlot.slot]||itemOrSlot.slot;return EQUIPMENT_TO_ASSET_SLOT[itemOrSlot]||itemOrSlot}
  ...
  const paperDollMasks=Object.fromEntries(PAPER_DOLL_POSES.map(pose=>[pose,Object.fromEntries(ASSET_GEAR_SLOTS.map(slot=>[slot,loadImage('assets/paper-doll/'+pose+'-'+slot+'.png')]))]));
  const GEAR_ATLAS_COLUMNS=10,GEAR_ATLAS_ROWS=5,GEAR_ATLAS_CELL=256,GEAR_ATLAS_ROW=Object.fromEntries(ASSET_GEAR_SLOTS.map((slot,row)=>[slot,row]));
  ...
  const LEGENDARY_SLOT_COLUMN=Object.fromEntries(ASSET_GEAR_SLOTS.map((slot,column)=>[slot,column]));
  const LEGENDARY_LEGACY_CELL={tyrantEmbercore:{row:4,column:3},wardenSingularity:{row:4,column:2}};
  function gearDef(id,name,slot,rarity,value,stats,color,accent,style,extra){let canonical=equipmentSlot(slot);return Object.assign({id,name,slot:canonical,assetSlot:slot,rarity,value,stats,form:canonical,visual:{color,accent,style:style||0}},extra||{})}
  ...
  const SET_ITEMS=SET_DEFINITIONS.flatMap((set,setIndex)=>ASSET_GEAR_SLOTS.map((slot,slotIndex)=>gearDef(set.id+'-'+slot,(SET_SPECIAL_SLOT_NAMES[set.id]&&SET_SPECIAL_SLOT_NAMES[set.id][slot])||(set.name+' '+SET_SLOT_NAMES[slot]),slot,set.rarity,Math.round((set.valueBase||(set.rarity==='legendary'?360:set.rarity==='epic'?92:55))*(1+slotIndex*.06)),fixedSetPieceStats(set,slot,slotIndex),set.color,set.accent,setIndex%6,{setId:set.id,minLevel:set.minLevel,setPiece:true,mark:set.mark,dropBand:set.dropBand||set.rarity})));
  // The pre-V1 catalog remains below as source history for balance reference only. It is
  // deliberately excluded from every active loot, inventory and developer path because
  // those images were not authored for the approved underwear-only V1 master.
  const ARCHIVED_GEAR_ITEMS=Object.freeze([...LEGACY_LOOT_ITEMS,...SET_ITEMS]);
  function productionAsset(atlasId,column,row,sourceId){let atlas=productionGearAtlases[atlasId];if(!atlas)return null;if(atlas.mode==='aligned')return Object.freeze({id:(sourceId||atlasId)+'@'+atlasId,atlasId,path:atlas.path,mode:'aligned',theme:atlas.theme,column:0,row:0,columns:1,rows:1,cell:0,sourceId:sourceId||null});return Object.freeze({id:(sourceId||atlasId)+'@'+atlasId+':'+column+':'+row,atlasId,path:atlas.path,mode:'atlas',column,row,columns:atlas.columns,rows:atlas.rows,cell:atlas.cell,sourceId:sourceId||null})}
  const gearVisualCounts={};
  for(const item of LEGACY_LOOT_ITEMS){
    let sourceSlot=assetSlot(item),variant=gearVisualCounts[sourceSlot]||0,legacyCell=LEGENDARY_LEGACY_CELL[item.id];gearVisualCounts[sourceSlot]=variant+1;item.visual.variant=variant;item.visual.atlasColumn=variant%GEAR_ATLAS_COLUMNS;item.visual.atlasRow=GEAR_ATLAS_ROW[sourceSlot];item.visual.mark=item.mark||GEAR_SLOT_META[item.slot].icon;item.visual.key=item.id;if(legacyCell){item.visual.legendaryRow=legacyCell.row;item.visual.legendaryColumn=legacyCell.column}item.visual.asset=legacyCell?productionAsset('legendary',legacyCell.column,legacyCell.row,item.id):productionAsset('field',item.visual.atlasColumn,item.visual.atlasRow,item.id)
  }
  const legacyAssets=Object.fromEntries(LEGACY_LOOT_ITEMS.map(item=>[item.id,item.visual.asset]));
  for(const item of SET_ITEMS){
    let sourceSlot=assetSlot(item),dedicated=['blackHole','stormrunner','lavaSet','natureSet'].includes(item.setId),legendaryRow=LEGENDARY_SET_ROW[item.setId],sourceId=SET_GEAR_SOURCE_IDS[item.setId]&&SET_GEAR_SOURCE_IDS[item.setId][sourceSlot];item.visual.variant=LEGENDARY_SLOT_COLUMN[sourceSlot];item.visual.mark=item.mark||GEAR_SLOT_META[item.slot].icon;item.visual.key=item.id;
    ...
    else if(legendaryRow!=null){item.visual.legendaryRow=legendaryRow;item.visual.legendaryColumn=LEGENDARY_SLOT_COLUMN[sourceSlot];item.visual.asset=productionAsset('legendary',item.visual.legendaryColumn,legendaryRow,item.id)}
  ...
  const stormcallerSet=Object.freeze(SET_DEFINITIONS.find(set=>set.id==='stormrunner'));
  const STORMCALLER_SOURCE_BY_SLOT=Object.freeze({cape:'stormcallerCape',legs:'stormcallerLegs',boots:'stormcallerBoots',chest:'stormcallerChest',scarf:'stormcallerScarf',hat:'stormcallerHat',weapon:'stormcallerWeapon'});
  function stormcallerIconPath(slot){return 'assets/equipment/stormcaller/icons/legendary_stormcaller_'+slot+'_icon_01.png?v=20260804-proof'}
  function activateStormcallerItem(source){let slot=source.slot,visual=Object.freeze(Object.assign({},source.visual,{asset:productionAsset(STORMCALLER_SOURCE_BY_SLOT[slot],0,0,source.id),iconPath:stormcallerIconPath(slot),mark:stormcallerSet.mark,key:source.id}));return Object.freeze(Object.assign({},source,{assetSlot:slot,visual}))}
  const stormcallerCoreItems=SET_ITEMS.filter(item=>item.setId==='stormrunner').map(activateStormcallerItem);
  const stormcallerCosmeticItems=[
    gearDef('stormrunner-cape','Stormwake Cape','cape','legendary',98,{},stormcallerSet.color,stormcallerSet.accent,0,{setId:'stormrunner',minLevel:stormcallerSet.minLevel,setPiece:true,cosmeticOnly:true,dropDisabled:true,mark:stormcallerSet.mark,dropBand:stormcallerSet.dropBand}),
    gearDef('stormrunner-legs','Thunderstride Legguards','legs','legendary',102,{},stormcallerSet.color,stormcallerSet.accent,0,{setId:'stormrunner',minLevel:stormcallerSet.minLevel,setPiece:true,cosmeticOnly:true,dropDisabled:true,mark:stormcallerSet.mark,dropBand:stormcallerSet.dropBand})
  ].map(activateStormcallerItem);
  const ACTIVE_SET_DEFINITIONS=Object.freeze([stormcallerSet]);
  const ACTIVE_SET_ITEMS=Object.freeze([...stormcallerCoreItems,...stormcallerCosmeticItems]);
  const LOOT_ITEMS=ACTIVE_SET_ITEMS;
  const LOOT_BY_RARITY=Object.keys(LOOT_RARITIES).reduce((groups,rarity)=>{groups[rarity]=LOOT_ITEMS.filter(item=>item.rarity===rarity&&!item.dropDisabled);return groups},{}),LOOT_BY_ID=Object.fromEntries(LOOT_ITEMS.map(item=>[item.id,item])),RELIC_POWER_CAP=4;
  const SYNERGIES=[
  ...
  const EQUIPMENT_LAYER_CELL=256,EQUIPMENT_LAYER_CACHE_LIMIT=36,INVENTORY_PAPER_DOLL_CELL=1024;
  ...
  function recordFramePerformance(frameMs){
    ...
    if(perfState.slowFor>1800&&index>0&&sinceSwitch>3000)applyQuality(QUALITY_ORDER[index-1]);
  ...
  function equippedGear(slot){slot=equipmentSlot(slot);let uid=save.equipped&&save.equipped[slot],gear=gearIndex().get(uid),def=gearDefinition(gear);return gear&&def&&def.slot===slot?gear:null}
  ...
  function equippedFullSetId(resolveItem){let resolver=resolveItem||equippedItem,items=SET_GEAR_SLOTS.map(resolver);if(items.some(item=>!item||!item.setId))return null;let setId=items[0].setId;return items.every(item=>item.setId===setId)?setId:null}
  ...
  const EQUIPPED_GEAR_PREVIEW_RULES=Object.freeze({
    ...
    cape:Object.freeze({padding:.08,anchorX:0,anchorY:0}),
    chest:Object.freeze({padding:.07,anchorX:0,anchorY:0}),
    legs:Object.freeze({padding:.08,anchorX:0,anchorY:0}),
    boots:Object.freeze({padding:.07,anchorX:0,anchorY:0}),
    weapon:Object.freeze({padding:.06,anchorX:0,anchorY:0}),
    necklace:Object.freeze({padding:.13,anchorX:0,anchorY:0}),
    ring1:Object.freeze({padding:.16,anchorX:0,anchorY:0}),
    ring2:Object.freeze({padding:.16,anchorX:0,anchorY:0})
  });
  function gearAssetRef(item){
    let asset=item&&item.visual&&item.visual.asset,atlas=asset&&productionGearAtlases[asset.atlasId],aligned=!!(asset&&asset.mode==='aligned'),valid=!!(asset&&atlas&&asset.path===atlas.path&&(aligned?atlas.mode==='aligned':Number.isInteger(asset.column)&&Number.isInteger(asset.row)&&asset.column>=0&&asset.column<atlas.columns&&asset.row>=0&&asset.row<atlas.rows));
    if(!valid&&item&&!missingGearAssetWarnings.has(item.id)){missingGearAssetWarnings.add(item.id);console.warn('[Gear Asset] Missing or invalid equip asset for '+item.id+' ('+(asset&&asset.path||'no asset path')+'); neutral Pappa Hammer fallback retained.')}
    return valid?asset:null
  }
  function paperDollAssetsReady(){return PAPER_DOLL_POSES.every(pose=>imageReady(pappaHammerSprites[pose])&&ASSET_GEAR_SLOTS.every(slot=>imageReady(paperDollMasks[pose][slot])))&&Object.values(productionGearAtlases).every(atlas=>imageReady(atlas.image))}
  const EQUIPMENT_VISUAL_CHANNELS=Object.freeze([
    Object.freeze({id:'cape',sourceSlot:'cape',maskSlot:'coat',region:'cape',layer:10,fit:'cover',anchor:EQUIPMENT_SLOT_ANCHORS.cape,position:'back'}),
    Object.freeze({id:'legs',sourceSlot:'legs',maskSlot:null,region:'legs',layer:20,fit:'cover',anchor:EQUIPMENT_SLOT_ANCHORS.legs,position:'front'}),
    Object.freeze({id:'boots',sourceSlot:'boots',maskSlot:'boots',region:'boots',layer:30,fit:'contain',anchor:EQUIPMENT_SLOT_ANCHORS.boots,position:'front'}),
    Object.freeze({id:'chest',sourceSlot:'chest',maskSlot:'coat',region:'chestArmor',layer:40,fit:'cover',anchor:EQUIPMENT_SLOT_ANCHORS.chest,position:'front'}),
    Object.freeze({id:'scarf',sourceSlot:'scarf',maskSlot:'scarf',region:'scarf',layer:50,fit:'contain',anchor:EQUIPMENT_SLOT_ANCHORS.scarf,position:'front'}),
    Object.freeze({id:'hat',sourceSlot:'hat',maskSlot:'hat',region:'hat',layer:60,fit:'contain',anchor:EQUIPMENT_SLOT_ANCHORS.hat,position:'front'}),
    Object.freeze({id:'weapon',sourceSlot:'weapon',maskSlot:'hammer',region:'weapon',layer:70,fit:'contain',anchor:EQUIPMENT_SLOT_ANCHORS.weapon,position:'front'})
  ]);
  ...
  function drawAnchoredEquipmentMask(layer,pose,channel,frame){
    ...
    else if(channel.id==='legs'){layer.moveTo(x+w*.22,y+h*.48);layer.quadraticCurveTo(x+w*.37,y+h*.5,x+w*.49,y+h*.57);layer.lineTo(x+w*.4,y+h*.84);layer.lineTo(x+w*.13,y+h*.79);layer.closePath();layer.moveTo(x+w*.49,y+h*.57);layer.quadraticCurveTo(x+w*.64,y+h*.49,x+w*.79,y+h*.48);layer.lineTo(x+w*.91,y+h*.78);layer.lineTo(x+w*.58,y+h*.84);layer.closePath()}
  ...
  function equipmentChannelMaskFrame(pose,channel,frame){
    ...
    let source=channel.maskSlot?paperDollMaskFrame(pose,channel.maskSlot,frame):null,canvas=document.createElement('canvas'),layer=canvas.getContext('2d');canvas.width=canvas.height=PAPER_DOLL_CELL;if(source)layer.drawImage(source.canvas,0,0);
    // The legacy coat mask is the shared alignment template for the independent cape and chest channels.
    if(channel.id==='cape')layer.clearRect(PAPER_DOLL_CELL*.44,0,PAPER_DOLL_CELL*.56,PAPER_DOLL_CELL);
    else if(channel.id==='chest'||channel.id==='legs'||channel.id==='scarf'||channel.id==='boots')drawAnchoredEquipmentMask(layer,pose,channel,frame);
  ...
  function gearAssetAlphaBounds(asset){
    let key=asset.id,cached=gearAssetBoundsCache.get(key);if(cached)return cached;
    let atlas=productionGearAtlases[asset.atlasId];if(!atlas||!imageReady(atlas.image))return null;let aligned=asset.mode==='aligned',sourceWidth=aligned?atlas.image.naturalWidth:asset.cell,sourceHeight=aligned?atlas.image.naturalHeight:asset.cell,canvas=document.createElement('canvas'),layer=canvas.getContext('2d',{willReadFrequently:true});canvas.width=sourceWidth;canvas.height=sourceHeight;layer.clearRect(0,0,sourceWidth,sourceHeight);if(aligned)layer.drawImage(atlas.image,0,0,sourceWidth,sourceHeight);else layer.drawImage(atlas.image,asset.column*asset.cell,asset.row*asset.cell,asset.cell,asset.cell,0,0,asset.cell,asset.cell);
    let pixels=layer.getImageData(0,0,sourceWidth,sourceHeight).data,minX=sourceWidth,minY=sourceHeight,maxX=-1,maxY=-1;for(let y=0;y<sourceHeight;y+=2)for(let x=0;x<sourceWidth;x+=2)if(pixels[(y*sourceWidth+x)*4+3]>8){if(x<minX)minX=x;if(y<minY)minY=y;if(x>maxX)maxX=x;if(y>maxY)maxY=y}
  ...
  function equippedGearPreviewTransform(asset,displaySlot){
    let size=EQUIPPED_GEAR_PREVIEW_SIZE,slot=equipmentSlot(displaySlot),rule=EQUIPPED_GEAR_PREVIEW_RULES[slot]||EQUIPPED_GEAR_PREVIEW_RULES.chest,cacheKey=asset.id+'|'+slot+'|'+size,cached=equippedGearPreviewTransformCache.get(cacheKey);if(cached)return cached;
    let source=gearAssetAlphaBounds(asset);if(!source)return null;
    let padding=Math.max(.04,Math.min(.2,rule.padding)),radius=size*(.5-padding),fit=radius*2/Math.max(1,Math.hypot(source.w,source.h)),width=source.w*fit,height=source.h*fit,anchorX=rule.anchorX||0,anchorY=rule.anchorY||0;
  ...
  function drawEquippedGearPreview(canvas,gear,displaySlot){
    ...
    canvas.width=canvas.height=EQUIPPED_GEAR_PREVIEW_SIZE;layer.clearRect(0,0,canvas.width,canvas.height);layer.save();layer.beginPath();layer.arc(canvas.width/2,canvas.height/2,transform.radius,0,Math.PI*2);layer.clip();layer.imageSmoothingEnabled=true;layer.imageSmoothingQuality='high';let source=transform.source,sourceX=(asset.mode==='aligned'?0:asset.column*asset.cell)+source.x,sourceY=(asset.mode==='aligned'?0:asset.row*asset.cell)+source.y;layer.drawImage(atlas.image,sourceX,sourceY,source.w,source.h,transform.x,transform.y,transform.width,transform.height);layer.restore();canvas.hidden=false;if(fallback)fallback.hidden=true;return true
  }
  ...
  function drawProductionGearAsset(layer,item,b,fitMode){
    ...
    let scale=(fitMode==='contain'?Math.min:Math.max)(b.w/source.w,b.h/source.h),width=source.w*scale,height=source.h*scale,x=b.x+(b.w-width)/2,y=b.y+(b.h-height)/2,sourceX=(asset.mode==='aligned'?0:asset.column*asset.cell)+source.x,sourceY=(asset.mode==='aligned'?0:asset.row*asset.cell)+source.y;layer.drawImage(atlas.image,sourceX,sourceY,source.w,source.h,x,y,width,height);return true
  }
  function drawAlignedProductionGearAsset(layer,asset,b){
    let atlas=productionGearAtlases[asset.atlasId],sourceBounds=pappaHammerSourceBounds;if(!atlas||!imageReady(atlas.image)||!sourceBounds||!b)return false;let fit=Math.min(b.w/sourceBounds.w,b.h/sourceBounds.h),x=b.x+(b.w-sourceBounds.w*fit)/2-sourceBounds.x*fit,y=b.y+b.h-sourceBounds.h*fit-sourceBounds.y*fit;layer.drawImage(atlas.image,0,0,atlas.image.naturalWidth,atlas.image.naturalHeight,x,y,atlas.image.naturalWidth*fit,atlas.image.naturalHeight*fit);return true
  }
  ...
  function cancelEquipmentPrewarm(){
    equipmentPrewarmGeneration++;equipmentPrewarmPending=0;if(equipmentPrewarmDelayHandle){clearTimeout(equipmentPrewarmDelayHandle);equipmentPrewarmDelayHandle=0}if(!equipmentPrewarmHandle)return;if(typeof cancelIdleCallback==='function')cancelIdleCallback(equipmentPrewarmHandle);else clearTimeout(equipmentPrewarmHandle);equipmentPrewarmHandle=0
  }
  function scheduleEquipmentPrewarm(delayMs){
    ...
    let run=deadline=>{
      equipmentPrewarmHandle=0;if(generation!==equipmentPrewarmGeneration)return;
      let completed=0,limit=mode==='run'?1:2;
      while(tasks.length&&completed<limit&&(completed===0||deadline.didTimeout||deadline.timeRemaining()>5)){let task=tasks.shift();try{task()}catch(error){console.warn('[Gear Warmup] '+error.message)}completed++}
      equipmentPrewarmPending=tasks.length;
      if(tasks.length){queueNext(run);return}
      if(generation!==equipmentPrewarmGeneration)return;paperDollKey='';if(mode!=='run'||ui.gearOverlay.classList.contains('show'))refreshPaperDoll()
    };
    queueNext(run)
  }
  ...
  function scaledPaperDollBounds(bounds,scale){return bounds?{x:bounds.x*scale,y:bounds.y*scale,w:bounds.w*scale,h:bounds.h*scale}:null}
  function drawHighResolutionBase(layer,target){
    if(!imageReady(pappaHammerImage)||!pappaHammerSourceBounds||!target)return false;let source=pappaHammerSourceBounds,fit=Math.min(target.w/source.w,target.h/source.h),width=source.w*fit,height=source.h*fit,x=target.x+(target.w-width)/2,y=target.y+target.h-height;layer.drawImage(pappaHammerImage,source.x,source.y,source.w,source.h,x,y,width,height);return true
  }
  function drawHighResolutionEquipmentLayer(layer,pose,channel,item,frame,target,cellX,cellY){
    let asset=gearAssetRef(item);if(!asset)return false;let atlas=productionGearAtlases[asset.atlasId];layer.save();layer.translate(cellX,cellY);
    if(asset.mode==='aligned'&&atlas&&imageReady(atlas.image)){let drawn=drawAlignedProductionGearAsset(layer,asset,target);layer.restore();return drawn}
    layer.restore();let texture=equipmentLayerTexture(pose,channel,item);if(!texture)return false;let sx=frame%4*EQUIPMENT_LAYER_CELL,sy=Math.floor(frame/4)*EQUIPMENT_LAYER_CELL;layer.drawImage(texture,sx,sy,EQUIPMENT_LAYER_CELL,EQUIPMENT_LAYER_CELL,cellX,cellY,INVENTORY_PAPER_DOLL_CELL,INVENTORY_PAPER_DOLL_CELL);return true
  }
  function highResolutionInventoryPaperDoll(resolveItem){
    let stormcallerFigure=stormcallerInventoryFigure(resolveItem);
    if(stormcallerFigure){
      let full=highResolutionStormcallerFigure(stormcallerFigure);
      if(full)return full
    }
    let resolver=resolveItem||equippedItem,pose='idle',scale=INVENTORY_PAPER_DOLL_CELL/PAPER_DOLL_CELL,out=document.createElement('canvas'),layer=out.getContext('2d');out.width=INVENTORY_PAPER_DOLL_CELL*4;out.height=INVENTORY_PAPER_DOLL_CELL*2;if(!layer)return'';layer.imageSmoothingEnabled=true;layer.imageSmoothingQuality='high';
    let back=EQUIPMENT_VISUAL_CHANNELS.filter(channel=>channel.position==='back'),front=EQUIPMENT_VISUAL_CHANNELS.filter(channel=>channel.position!=='back').sort((a,b)=>a.layer-b.layer);
    for(let frame=0;frame<8;frame++){let cellX=frame%4*INVENTORY_PAPER_DOLL_CELL,cellY=Math.floor(frame/4)*INVENTORY_PAPER_DOLL_CELL,target=scaledPaperDollBounds(equipmentBodyTargetBounds(pose,frame),scale);for(const channel of back){let item=resolver(channel.sourceSlot);if(item)drawHighResolutionEquipmentLayer(layer,pose,channel,item,frame,target,cellX,cellY)}layer.save();layer.translate(cellX,cellY);drawHighResolutionBase(layer,target);layer.restore();for(const channel of front){let item=resolver(channel.sourceSlot);if(item)drawHighResolutionEquipmentLayer(layer,pose,channel,item,frame,target,cellX,cellY)}}
    return 'url("'+out.toDataURL('image/png')+'")'
  }
  function stormcallerInventoryFigure(resolveItem){
    let resolver=resolveItem||equippedItem,core=['hat','scarf','chest','boots'],coreReady=core.every(slot=>{let item=resolver(slot);return item&&item.setId==='stormrunner'});if(!coreReady)return null;
    let weapon=resolver('weapon');if(weapon&&weapon.setId==='stormrunner')return stormcallerFullFigure;if(!weapon)return stormcallerWeaponlessFigure;return null
  }
  function stormcallerInventoryFigureVariant(resolveItem){
    let figure=stormcallerInventoryFigure(resolveItem);return figure===stormcallerFullFigure?'full':figure===stormcallerWeaponlessFigure?'weaponless':'modular'
  }
  ...
  function safeHighResolutionInventoryPaperDoll(resolveItem){
    try{return highResolutionInventoryPaperDoll(resolveItem)}catch(error){return''}
  }
  ...
  function applyPaperDollLayers(key,background){
    paperDollAtlases=Object.assign({},pappaHammerSprites);paperDollKey=key;paperDollCurrentUrl=background;let visualKey=paperDollLoadoutKey();ui.pappaHammerBaseSprite.style.backgroundImage=background;ui.gearCharacterHero.style.backgroundImage=background;ui.gearCharacterHero.dataset.gearVisualKey=visualKey;if(ui.combatPortraitSprite)ui.combatPortraitSprite.style.backgroundImage=background
  }
  ...
  function synchronizeEquipmentPreview(){
    ...
    refreshPaperDoll();
    if(!paperDollCurrentUrl){let fallback=pappaHammerSprites.idle&&pappaHammerSprites.idle.src?'url("'+pappaHammerSprites.idle.src+'")':'';paperDollCurrentUrl=fallback;if(fallback){ui.pappaHammerBaseSprite.style.backgroundImage=fallback;ui.gearCharacterHero.style.backgroundImage=fallback;if(ui.combatPortraitSprite)ui.combatPortraitSprite.style.backgroundImage=fallback}}
  ...
  function applyEquippedPaperDoll(){
    if(!ui.gearCharacterHero)return;if(paperDollCurrentUrl&&ui.gearCharacterHero.style.backgroundImage!==paperDollCurrentUrl)ui.gearCharacterHero.style.backgroundImage=paperDollCurrentUrl;applyInventoryBackdrop(ui.gearCharacterStage,inventoryBackdropSetId())
  }
  ...
  function equippedSetCounts(){let counts={};for(const slot of SET_GEAR_SLOTS){let def=equippedItem(slot);if(def&&def.setId)counts[def.setId]=(counts[def.setId]||0)+1}return counts}
  ...
  function gearArtMarkup(gear,size){
    ...
    if(size)classes.push(size);if(item.setId)classes.push('setItem');if(item.rarity==='legendary')classes.push('legendarySprite','legendary-'+(item.setId||'legacy'));let theme=asset&&asset.theme||asset&&asset.atlasId;if(theme&&['blackHole','stormrunner','lavaSet','natureSet'].includes(theme))classes.push(theme==='stormrunner'?'stormcallerSprite':theme==='lavaSet'?'lavaSprite':theme==='natureSet'?'natureSprite':'blackHoleSprite');if(!asset)classes.push('neutralGearFallback');
    let x=asset&&asset.columns>1?asset.column/(asset.columns-1)*100:0,y=asset&&asset.rows>1?asset.row/(asset.rows-1)*100:0,mark=String(item.mark||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;'),iconPath=visual.iconPath||asset&&asset.path,background=asset&&asset.mode==='aligned'?'background-image:url(\''+iconPath+'\');background-size:contain;background-repeat:no-repeat;background-position:center;':asset?'background-image:url(\''+asset.path+'\');background-size:'+(asset.columns*100)+'% '+(asset.rows*100)+'%;background-position:'+x.toFixed(3)+'% '+y.toFixed(3)+'%;':'';
  ...
  function rollBossGear(level,forcedRarity,highestDepth){
    if(!LOOT_ITEMS.length)return null;
    level=progression.clampLevel(level||save.level);
    let forced=Boolean(forcedRarity),context=lootProgressionContext(level,highestDepth==null?highestLootDepth():highestDepth),band=forcedRarity||bossGearBand(level,context.highestDepth),pool=LOOT_ITEMS.filter(item=>!item.dropDisabled&&lootItemBand(item)===band&&(item.minLevel||1)<=(forced?level:context.effectiveLevel)&&(forced||lootRarityEligible(item.rarity,context)));
    if(!pool.length&&!forced){for(const fallback of ['legendary','epic','rare','common']){if(!lootRarityEligible(fallback,context))continue;pool=LOOT_ITEMS.filter(item=>!item.dropDisabled&&item.rarity===fallback&&(item.minLevel||1)<=context.effectiveLevel);if(pool.length)break}}
    if(!pool.length&&forced)pool=LOOT_ITEMS.filter(item=>!item.dropDisabled&&item.rarity===forcedRarity&&(item.minLevel||1)<=level);
    if(!pool.length)pool=LOOT_ITEMS.filter(item=>!item.dropDisabled&&item.rarity==='common');
    let def=pool[Math.floor(Math.random()*pool.length)];if(!def)return null;let itemLevel=progression.clampLevel(level+(Math.random()<.18?1:Math.random()<.28?-1:0)),quality=.84+Math.random()*.3+Math.min(.06,riskTier*.012);return rollGearInstance(def,itemLevel,Math.min(1.35,quality))
  }
  ...
  function rollVaultGear(){let gear=rollBossGear(save.level,vaultDropBand());if(!gear)return null;let def=gearDefinition(gear);gear.quality=Math.min(1.35,Math.max(1.02,gear.quality+.08));let scale=gearScaleForLevel(gear.level)*gear.quality,stats={};for(const key of Object.keys(def.stats||{}))stats[key]=roundGearStat(key,def.stats[key]*scale*(.97+Math.random()*.1));gear.stats=stats;gear.value=Math.max(gear.value,Math.round(def.value*gearValueScaleForLevel(gear.level)*gear.quality*1.18));return gear}
  ...
  function hideGearHover(uid){if(uid&&hoverGearUid!==uid)return;let active=hoverGearUid||gearHoverPreview.classList.contains('show');if(!active)return;hoverGearUid=null;gearHoverPreview.classList.remove('show','touchPreview','legendaryPreview');gearHoverPreview.style.bottom='';gearHoverPreview.setAttribute('aria-hidden','true')}
  ...
  function gearQuickFeedback(uid,slot,rarity,removing){
    ...
    for(const target of targets){target.style.setProperty('--gear-color',rarity.color);target.classList.remove('gearQuickEquip','gearQuickRemove');requestAnimationFrame(()=>{if(!target.isConnected)return;target.classList.add(removing?'gearQuickRemove':'gearQuickEquip');setTimeout(()=>target.classList.remove('gearQuickEquip','gearQuickRemove'),360)})}
  }
  ...
  function playGearEquipEffect(gear,awakened,fullSetActivated){
    ...
    if(fullSetActivated){
      ...
      setTimeout(()=>ui.gearCharacterStage.classList.remove('fullSetMorph'),720);
      return
    }
  ...
  function equipGear(uid){
    ...
    equipPerfStep(record,'saveQueue',()=>scheduleEquipPersist(record));equipPerfMark(record,'saveQueued');refreshEquippedGearUI(record,item.slot);
  ...
  function refreshArmoryCharacter(){
    ...
    refreshPaperDoll();let stormcallerFigure=stormcallerInventoryFigure(),inventoryFigure=safeHighResolutionInventoryPaperDoll(),usesStaticFigure=!!(stormcallerFigure&&inventoryFigure);paperDollCurrentUrl=inventoryFigure||paperDollCurrentUrl;ui.gearCharacterHero.classList.toggle('staticInventoryFigure',usesStaticFigure);ui.gearCharacterStage.classList.toggle('staticInventoryFigureMode',usesStaticFigure);applyEquippedPaperDoll();applyLoadoutRarity(ui.gearCharacterStage,profile);applyLoadoutSetVisual(ui.gearCharacterStage,fullSetId);applyInventoryBackdrop(ui.gearCharacterStage,inventoryBackdropSetId());ui.gearLoadoutName.textContent=equipped.length+'/'+GEAR_SLOTS.length+' EQUIPPED'+(bestSet&&sets[bestSet]>=2?' \u00B7 '+SET_BY_ID[bestSet].name:highestDef?' \u00B7 '+LOOT_RARITIES[highestDef.rarity].name:'');ui.gearEquippedCount.textContent=equipped.length+' / '+GEAR_SLOTS.length;ui.gearPreviewName.textContent=highestDef?highestDef.name.toUpperCase():'FIELD LOADOUT';ui.gearPreviewName.style.setProperty('--loadout-color',profile.color);if(paperDollCurrentUrl)ui.gearCharacterHero.style.backgroundImage=paperDollCurrentUrl;ui.gearCharacterHero.dataset.gearVisualKey=paperDollLoadoutKey()
  }
  ...
  function activateGearEntry(event,gear){
    ...
    clearGearActionConfirm();selectedGearUid=gear.uid;gearTapUid=touch?gear.uid:null;gearTapAt=touch?now:0;toggleBulkGear(gear.uid)
  }
  function renderGearBuildIdentity(){
    let counts=equippedSetCounts(),bestSetId=Object.keys(counts).sort((a,b)=>counts[b]-counts[a])[0],set=bestSetId&&SET_BY_ID[bestSetId],count=set?counts[bestSetId]:0,signature=set&&GEAR_SIGNATURES[set.id],next=set&&nextSetMilestone(set,count),equipped=GEAR_SLOTS.map(equippedGear).filter(Boolean).length,color=signature?signature.color:set?set.accent:'#d6aa58';
    ui.gearBuildName.textContent=set?set.name.toUpperCase()+' SET':'FIELD LOADOUT';
    ui.gearBuildProgress.textContent=set?count+' / 5':equipped+' / '+GEAR_SLOTS.length;
    ui.gearBuildBonus.textContent=set?(count>=5?(signature?signature.name.toUpperCase()+' ACTIVE':'FULL SET BONUS ACTIVE'):next.label+' \u00B7 '+next.effect):equipped?'MIXED GEAR':'NO SET BONUS';
    ui.gearBuildName.style.setProperty('--build-color',color);ui.gearBuildProgress.style.setProperty('--build-color',color)
  }
  function renderGearProgressAndStats(){
    ...
    let stats=gearStats();ui.gearStatsSummary.innerHTML='<span data-help-title="Health" data-help="Maximum damage Pappa Hammer can survive before unsecured expedition loot is lost."><small>HEALTH</small><b>'+maxHp()+'</b></span><span data-help-title="Damage" data-help="Power of close-range hammer impacts and physical shockwaves before critical hits."><small>DAMAGE</small><b>'+shotDamage()+'</b></span><span data-help-title="Reach" data-help="Distance from which coins, caches and nearby pickups are drawn toward Pappa Hammer."><small>REACH</small><b>'+magnetRange()+'</b></span><span data-help-title="Impact Speed" data-help="Faster close-range hammer strikes and faster Hammerstorm pulses."><small>IMPACT</small><b>+'+Math.round(stats.fire*100)+'%</b></span><span data-help-title="Critical Chance" data-help="Chance for a melee impact to deal amplified critical damage."><small>CRIT</small><b>'+Math.round(stats.crit*100)+'%</b></span>';renderGearBuildIdentity();
  }
  function renderGearLoadoutSlots(changedSlot){
    gearPerf.slotRenders++;let displays=GEAR_SLOTS.map(slot=>({slot,display:slot,name:GEAR_SLOT_META[slot].name}));
    ...
    for(const display of displays){
      ...
      button.setAttribute('aria-label',item?display.name+': '+item.name+'. Tap to remove.':'Empty '+display.name+' slot');if(gear){button.setAttribute('data-item',gear.uid);button.setAttribute('data-info-ready','');button.draggable=!mobileArmory()}else{button.setAttribute('data-help-title','Empty '+display.name+' Slot');button.setAttribute('data-help',meta.visual?'Equip compatible '+meta.name.toLowerCase()+' gear here.':'This stat slot is ready for '+meta.name.toLowerCase()+' gear and does not add a character layer.')}
  ...
  function updateGearToolbarState(items){
    let sort=GEAR_SORTS.find(entry=>entry.id===gearSort)||GEAR_SORTS[0],rarity=gearRarityFilter==='all'?'':LOOT_RARITIES[gearRarityFilter].name+' ',slotName=gearFilter==='all'?'ALL GEAR':GEAR_SLOT_META[gearFilter]&&GEAR_SLOT_META[gearFilter].name||'ALL GEAR';ui.gearSortLabel.textContent=sort.name;ui.mobileGearSortLabel.textContent=sort.name;ui.mobileGearFilterLabel.textContent=rarity+slotName;ui.gearInventorySummary.textContent='SHOWING '+items.length+' OF '+inventoryCount()+'  \u00B7  BAG VALUE $'+inventoryValue();renderSellFilteredButton()
  }
  ...
  function openGearLocker(slot){clearGearDropState();clearGearBulkSelection();gearFilter=slot||'all';gearRarityFilter='all';selectedGearUid=(GEAR_SLOTS.map(equippedGear).filter(Boolean).sort((a,b)=>gearScore(b)-gearScore(a))[0]||save.gear[0]||{}).uid||null;resetFilteredSaleArm();setGearView();renderGearLocker();ui.gearOverlay.classList.add('show');if(document.documentElement)document.documentElement.classList.add('gearPanelOpen');scheduleEquipmentPrewarm();if(save.settings.sound)setTimeout(ensureAudio,0)}
  ...
  function closeGearLocker(){flushEquipPersist();clearGearDropState();clearGearBulkSelection();hideGearHover();closeGearMobileSheet();ui.gearOverlay.classList.remove('show');if(document.documentElement)document.documentElement.classList.remove('gearPanelOpen');paperDollCurrentUrl=equipmentCssBackground('idle');ui.gearCharacterHero.classList.remove('staticInventoryFigure');ui.gearCharacterStage.classList.remove('staticInventoryFigureMode');ui.gearCharacterHero.style.backgroundImage=paperDollCurrentUrl;refreshBase()}
  ...
  function renderVaultRewards(){
    if(!vaultRewards.length){
      ui.contractOverlay.style.setProperty('--vault-color','#d6aa58');ui.vaultReward.innerHTML='<article class="vaultRewardMain foundationReward"><div aria-hidden="true">&#9874;</div><span><small>PAPPA V1 FOUNDATION</small><b>FORGE MATERIAL CACHE</b><em>Reserved for the first new gear layers</em><strong>+'+vaultMaterialReward+' MATERIALS</strong></span></article>';ui.vaultReward.classList.add('revealed');ui.vaultDescription.textContent='The old gear is retired. These materials are secured for Pappa Hammer\'s new equipment.';ui.vaultOdds.textContent='NEXT VAULT  \u00B7  3 NEW SEALS  \u00B7  12 NEW RELICS';ui.closeContract.disabled=false;ui.vaultClaimLabel.textContent='SECURE '+vaultMaterialReward+' MATERIALS';ui.contractOverlay.classList.remove('opening');ui.contractOverlay.classList.add('revealed');sound('rare');return
    }
  ...
  function openGrandVault(){
    if(mode!=='base'||!contractReady()||vaultOpening||ui.contractOverlay.classList.contains('show'))return;vaultOpening=true;let first=rollVaultGear();vaultRewards=first?[first]:[];if(first&&Math.random()<vaultExtraChance()){let bonus=rollVaultGear();if(bonus)vaultRewards.push(bonus)}vaultRewards.sort(compareGearPriority);vaultMaterialReward=vaultRewards.length?0:PAPPA_V1_FOUNDATION.vaultMaterialBase+Math.min(30,save.vaultCycle*3);ui.vaultCycle.textContent=String(save.vaultCycle+1).padStart(2,'0');ui.vaultEyebrow.textContent='GRAND VAULT  \u00B7  CYCLE '+(save.vaultCycle+1);ui.contractCompleteTitle.textContent='THE GRAND VAULT AWAKENS';ui.vaultDescription.textContent='The seals are turning. Something valuable is waiting inside.';ui.vaultOdds.textContent=vaultRewards.length?'GUARANTEED GEAR  \u00B7  '+Math.round(vaultExtraChance()*100)+'% BONUS ITEM':'V1 FORGE CACHE  \u00B7  PERMANENT MATERIALS';ui.vaultReward.classList.remove('revealed');ui.vaultReward.innerHTML='<span>UNLOCKING THE VAULT</span>';ui.closeContract.disabled=true;ui.vaultClaimLabel.textContent='OPENING...';ui.contractOverlay.classList.remove('revealed');ui.contractOverlay.classList.add('show','opening');sound('upgrade');setTimeout(renderVaultRewards,900)
  }
  function claimContract(){
    if(vaultOpening&&ui.closeContract.disabled||!vaultRewards.length&&!vaultMaterialReward)return;let legendary=0,count=vaultRewards.length,materials=vaultMaterialReward;batchEquipmentChanges(()=>{for(const gear of vaultRewards){let item=gearDefinition(gear);save.gear.push(gear);save.lootFound[gear.itemId]=(save.lootFound[gear.itemId]||0)+1;save.stats.itemsRecovered++;if(item.rarity==='legendary'){save.stats.legendaryRecovered++;legendary++}if(!save.equipped[item.slot])save.equipped[item.slot]=gear.uid}});if(materials)save.materials+=materials;save.vaultCycle++;save.stats.vaultsOpened++;save.contractSeen=true;save.contractComplete=false;selectedGearUid=count?vaultRewards[0].uid:null;vaultRewards=[];vaultMaterialReward=0;vaultOpening=false;persist();ui.contractOverlay.classList.remove('show','opening','revealed');refreshBase();notice(materials?'GRAND VAULT  \u00B7  +'+materials+' MATERIALS':('GRAND VAULT  \u00B7  '+count+' ITEM'+(count===1?'':'S')+' SECURED'),legendary?'#ffc928':'#d6aa58');sound(legendary?'legendary':'rare')
  }
  ...
  function lightningTravelEffects(){
    ...
    let afterimages=perfState.active==='high'?(mobileArmory()?2:3):perfState.active==='medium'?1:0;
    if(afterimages>0)spawnLightningEffect({kind:'lightningAfterimage',x:player.lightningStartX,y:player.lightningStartY,r:0,maxR:0,life:.15,max:.15,color:'#e9fdff',facing:player.facing,intensity});
    if(afterimages>1)spawnLightningEffect({kind:'lightningAfterimage',x:player.lightningStartX+dx*.4,y:player.lightningStartY+dy*.4,r:0,maxR:0,life:.11,max:.11,color:'#79e7f2',facing:player.facing,intensity});
    if(afterimages>2)spawnLightningEffect({kind:'lightningAfterimage',x:player.lightningStartX+dx*.72,y:player.lightningStartY+dy*.72,r:0,maxR:0,life:.08,max:.08,color:'#ffffff',facing:player.facing,intensity})
  }
  ...
  function bossLootCharacterImage(){
    let current=safeHighResolutionInventoryPaperDoll();if(current)return current;let atlas=paperDollAtlases.idle||pappaHammerSprites.idle;return atlas?(typeof atlas.toDataURL==='function'?'url("'+atlas.toDataURL('image/png')+'")':'url("'+atlas.src+'")'):''
  }
  ...
  function showBossLootPath(){
    bossLootPhase='path';let equipped=bossLootResolved.filter(result=>result.action==='equip').length,kept=bossLootResolved.filter(result=>result.action==='keep').length,salvaged=bossLootResolved.filter(result=>result.action==='salvage').length,nextLevel=progression.clampLevel(save.level+(expeditionCycle+1)*2),nextOdds=bossGearOdds(nextLevel,riskTier+2),best=bossLootRewards[0],bestItem=gearDefinition(best),rarity=bestItem?LOOT_RARITIES[bestItem.rarity]:{color:'#d6aa58',rank:2};
    if(!best){ui.bossLootPanel.style.setProperty('--loot-color',rarity.color);ui.bossLootPanel.className='bossLootPanel pathPhase foundationPhase';ui.bossLootDecision.setAttribute('aria-hidden','true');ui.bossLootPath.setAttribute('aria-hidden','false');ui.bossLootEyebrow.textContent='CHAMPION FORGE CACHE  \u00B7  FLOOR '+expeditionFloor();ui.bossLootTitle.textContent='CHOOSE YOUR RISK';ui.bossLootCount.textContent='+'+bossLootMaterialReward+' FORGE MATERIALS RECOVERED';ui.bossLootGrid.innerHTML='<article class="bossLootItem receipt kept foundationReceipt"><i aria-hidden="true">&#9874;</i><small>V1 FORGE CACHE</small><b>+'+bossLootMaterialReward+'</b></article>';ui.bossLootValue.textContent='$'+runScrap+'  \u00B7  '+save.materials+' MATERIALS';ui.bossLootMultiplier.textContent='NEW PAPPA HAMMER GEAR IS BEING FORGED';return}
  ...
  function showBossLootRitual(rewards,def,headline,materialReward){
    cancelSkillGesture('loot decision');let sorted=rewards.filter(Boolean).slice().sort(compareGearPriority);bossLootRewards=sorted;bossLootSelected=0;bossLootResolved=[];bossLootMaterialReward=Math.max(0,Math.floor(materialReward||0));postBossDecision=true;ui.bossLootGrid.innerHTML='';ui.bossLootOverlay.classList.remove('show');if(sorted.length)renderBossLootDecision();else{if(bossLootMaterialReward){save.materials+=bossLootMaterialReward;persist();refreshResourceCounters()}bossLootResolved=[{action:'materials',materials:bossLootMaterialReward}];showBossLootPath()}void ui.bossLootOverlay.offsetWidth;ui.bossLootOverlay.classList.add('show');ui.lootToast.classList.remove('show','legendary')
  }
  function finishBossLootChest(){
    if(!bossLootChest)return;let chest=bossLootChest,rewards=chest.rewards,def=chest.def,headline=chest.headline,materials=chest.materials;bossLootChest=null;showBossLootRitual(rewards,def,headline,materials)
  }
  ...
  function defeatBoss(e,xpReward){let def=BOSSES[e.bossKind]||currentBoss(),map=activeMap(),beforeLevel=xpReward?xpReward.beforeLevel:save.level,levels=xpReward?xpReward.levelsGained:0,newMap=EXPEDITION_MAP_IDS.map(id=>EXPEDITION_MAPS[id]).find(entry=>entry.minLevel>beforeLevel&&entry.minLevel<=save.level),stats=cargoStats(),markedDrop=Math.random()<stats.bossDropChance?1:0,mapDrop=Math.random()<map.dropBonus?1:0,drops=Math.min(6,2+Math.min(2,expeditionCycle)+(riskTier>=2?1:0)+(Math.random()<.16?1:0)+markedDrop+mapDrop),gearLevel=progression.clampLevel(save.level+expeditionCycle*2),rewards=[];for(let i=0;i<drops;i++){let reward=rollBossGear(gearLevel);if(reward)rewards.push(reward)}let best=rewards.slice().sort(compareGearPriority)[0],bestItem=gearDefinition(best),rarity=bestItem?LOOT_RARITIES[bestItem.rarity]:{color:'#d6aa58',name:'FORGE',rank:2},materialReward=best?0:PAPPA_V1_FOUNDATION.bossMaterialBase+drops*2+Math.min(10,riskTier*2+expeditionCycle*2),dropPoint=openArenaPosition(e.x,e.y,42);bossActive=false;bossDefeated=true;bossEntity=null;lockEnemySpawning();bossRunClears++;if(runStats)runStats.bosses=bossRunClears;save.stats.bosses++;persist();ui.bossHud.classList.remove('show');ui.cargoHud.classList.remove('bossHidden');ui.depthRoute.classList.remove('bossHidden');clearEnemyProjectiles();hazards=[];let cleanupCount=livingRegularEnemyCount();burst(e.x,e.y,def.accent,52,2.35);effects.push({x:e.x,y:e.y,r:18,maxR:230,life:.78,max:.78,color:def.accent});bossLootChest={x:dropPoint.x,y:dropPoint.y,r:38,spin:0,arrival:BOSS_LOOT_ORB_ARRIVAL,arrivalMax:BOSS_LOOT_ORB_ARRIVAL,cleanupLocked:cleanupCount>0,opening:0,opened:false,burstAt:0,rewards,materials:materialReward,def,headline:newMap?'NEW MAP UNLOCKED':levels?'PAPPA LEVEL '+save.level:'CHAMPION SPOILS',color:rarity.color,rarity:best?rarity.name:'FORGE CACHE',rank:rarity.rank};shake=Math.max(shake,18);flash=.09;setMusicMode('run');updateZoneHud();updateRouteHud();runNotice(cleanupCount?'CLEAR THE FIELD  \u00B7  '+cleanupCount+' REMAIN':(best?rarity.name+' LOOT ORB':'FORGE CACHE'),cleanupCount?'#f4ead6':rarity.color);sound(best&&rarity.rank>=4?'legendary':'boss')}
  ...
  function drawStormcallerAura(){
    let t=runTime||performance.now()/1000,active=player&&player.lightningPhase!=='idle',tempo=player?Math.min(1,player.lightningTempo||0):0,quality=perfState.active,reduced=!save.settings.particles||quality==='low',energy=active?.72+tempo*.28:.38+Math.sin(t*2.7)*.06,arcCount=reduced?1:quality==='medium'?(active?2:1):active?4:2;
    ctx.save();
    ...
    ctx.strokeStyle='#63ddff';ctx.shadowColor='#2f9dff';ctx.shadowBlur=reduced?0:quality==='medium'?4:9;ctx.lineWidth=1.2+energy;
  ...
  function drawEnemyArt(e,drawShadow){
    ...
    let lightweightHit=e.hit>0&&(denseSpinHit||perfState.active!=='high');
    ctx.save();ctx.globalAlpha=.45+spawn*.55;ctx.translate(-Math.cos(e.angle)*recoil*6,bob-recoil*1.5);ctx.rotate(lean);ctx.scale(.82+spawn*.18,.82+spawn*.18);if(drawShadow){ctx.fillStyle='rgba(3,7,14,.32)';ctx.beginPath();ctx.ellipse(0,e.r*.72,e.r*(e.type==='brute'?1.15:.88),e.r*.34,0,0,Math.PI*2);ctx.fill()}if(e.elite){let markerY=-box*.51-5;ctx.fillStyle='#d6aa58';ctx.strokeStyle='#111827';ctx.lineWidth=1.5;for(let i=-1;i<=1;i++){let x=i*9,y=markerY+Math.abs(i)*2,s=i===0?5:3.5;ctx.beginPath();ctx.moveTo(x,y-s);ctx.lineTo(x+s,y);ctx.lineTo(x,y+s);ctx.lineTo(x-s,y);ctx.closePath();ctx.fill();ctx.stroke()}}if(imageReady(enemyAtlas))drawAtlasCell(enemyAtlas,cell[0],cell[1],2,2,box,box,flip,e.hit>0&&!lightweightHit);else drawEnemyFallback(e);if(lightweightHit){ctx.globalAlpha=Math.min(.72,.25+e.hit*3.7);ctx.strokeStyle='#f4ead6';ctx.lineWidth=2.2;ctx.beginPath();ctx.arc(0,0,e.r*(1.02+e.hit*.8),0,Math.PI*2);ctx.stroke()}if(e.recover>0){let color=e.type==='brute'?'#ef746c':e.type==='lancer'?'#9eb2d5':'#d6aa58',amount=e.type==='brute'?3:2;ctx.globalAlpha=.55+Math.sin(e.anim*12)*.2;ctx.fillStyle=color;ctx.font='900 '+Math.max(8,e.r*.5)+'px Georgia';ctx.textAlign='center';for(let i=0;i<amount;i++){let a=-Math.PI*.82+i*Math.PI*.64/(amount-1||1);ctx.fillText('\u2726',Math.cos(a)*e.r*.72,Math.sin(a)*e.r*.68-e.r*.86)}}ctx.restore()
  }
  ...
  function draw(){
    ...
    for(const e of enemies){
      ...
      visibleEnemies.push({enemy:e,point:p})
    }
    ctx.save();ctx.fillStyle='rgba(3,7,14,.32)';ctx.beginPath();for(const visible of visibleEnemies){let e=visible.enemy,p=visible.point;if(e.boss)continue;ctx.moveTo(p.x+e.r*(e.type==='brute'?1.15:.88),p.y+e.r*.72);ctx.ellipse(p.x,p.y+e.r*.72,e.r*(e.type==='brute'?1.15:.88),e.r*.34,0,0,Math.PI*2)}ctx.fill();ctx.restore();
    for(const visible of visibleEnemies){
      let e=visible.enemy,p=visible.point;
      if(e.charge>0){if(e.type==='brute')drawTankTelegraph(e,p);else drawEnemyAimTelegraph(e,p)}
      ctx.save();ctx.translate(p.x,p.y);
      if(e.boss){drawBossStagger(e);drawBossArt(e)}else drawEnemyArt(e,false);
  ...
  function syncCombatHud(stats){
    ...
    syncText(ui.combatSetProgress,set?pieces+'/5':GEAR_SLOTS.filter(slot=>equippedItem(slot)).length+'/'+GEAR_SLOTS.length);
  ...
  function renderDevGearOptions(){
    let selected=ui.devGearSelect.value,sets=ACTIVE_SET_DEFINITIONS.slice().sort((a,b)=>LOOT_RARITIES[b.rarity].rank-LOOT_RARITIES[a.rarity].rank||a.name.localeCompare(b.name)),items=LOOT_ITEMS.slice().sort((a,b)=>LOOT_RARITIES[b.rarity].rank-LOOT_RARITIES[a.rarity].rank||a.slot.localeCompare(b.slot)||a.name.localeCompare(b.name));
    ui.devGearSelect.innerHTML='<option value="">'+(items.length?'SELECT GEAR':'NO V1 GEAR REGISTERED')+'</option>';
    ui.devGearSelect.disabled=!items.length;ui.devGearSpawn.disabled=!items.length;ui.devGearEquip.disabled=!items.length;
    ...
    else{let equipped=equippedFullSetId();ui.devGearSelect.value=equipped&&sets.some(set=>set.id===equipped)?'set:'+equipped:''}
  }
  ...
  function devGrantGear(equip){
    let [kind,id]=(ui.devGearSelect.value||'').split(':'),items=kind==='set'?ACTIVE_SET_ITEMS.filter(item=>item.setId===id):kind==='item'?[LOOT_BY_ID[id]].filter(Boolean):[];if(!items.length){devFeedback('NEW V1 GEAR IS NOT READY YET');return}
  ...
  function installInventoryV2Bridge(){
    ...
    function snapshot(){
      let counts=equippedSetCounts(),copies=gearCopyCounts();
  ...
  function installPlaywrightBridge(){
    ...
    window.__riskTest={
      ...
      rollGearLevels(levels){
        let def=LOOT_ITEMS[0];if(!def)return[];return (Array.isArray(levels)?levels:[]).map(level=>{let gear=rollGearInstance(def,level,1);return{requested:level,level:gear.level,value:gear.value,finite:Object.values(gear.stats).every(Number.isFinite)}})
      },
      levelScaling(level){
      ...
      sampleBossLoot(level,highestDepth,count){
        return Array.from({length:Math.max(1,Math.min(500,Number(count)||1))},()=>{let gear=rollBossGear(level,null,highestDepth),item=gearDefinition(gear);return item?item.rarity:'materials'})
      },
      ...
      gearSetRules(){
        return Object.fromEntries(ACTIVE_SET_DEFINITIONS.map(set=>[set.id,{tiers:setBonusTiers(set).slice(),signaturePieces:setSignaturePieces(set)}]))
      },
      devGearState(){
      ...
      spawnHammerstormPack(count,options){
        ...
        if(options.fullRiskreaver||options.fullBlackHole||options.fullLightning||options.fullLava||options.fullNature){
          ...
          let fullSetItems=options.fullLightning?ACTIVE_SET_ITEMS.filter(entry=>entry.setId===set.id):SET_GEAR_SLOTS.map(slot=>SET_ITEMS.find(entry=>entry.setId===set.id&&entry.slot===slot)).filter(Boolean);for(const slot of GEAR_SLOTS)save.equipped[slot]=null;for(const item of fullSetItems){let gear=rollGearInstance(item,Math.max(set.minLevel,save.level),1.08);save.gear.push(gear);save.equipped[item.slot]=gear.uid}
      ...
      prepareHandlava(points,pieces){
        let set=SET_BY_ID.lavaSet,count=Math.max(0,Math.min(5,Math.floor(pieces==null?5:Number(pieces)||0))),slots=['hat','scarf','chest','boots','weapon'];
      ...
      prepareNature(points,pieces){
        let set=SET_BY_ID.natureSet,count=Math.max(0,Math.min(5,Math.floor(pieces==null?5:Number(pieces)||0))),slots=['hat','scarf','chest','boots','weapon'];
        for(const slot of GEAR_SLOTS)save.equipped[slot]=null;
      ...
      previewGearSet(setId){
        let set=ACTIVE_SET_DEFINITIONS.find(entry=>entry.id===setId);if(!set)return null;
        batchEquipmentChanges(()=>{for(const slot of GEAR_SLOTS)save.equipped[slot]=null;for(const item of ACTIVE_SET_ITEMS.filter(entry=>entry.setId===setId)){
          let gear=rollGearInstance(item,Math.max(set.minLevel,save.level),1.08);
          save.gear.push(gear);save.equipped[item.slot]=gear.uid
        }});
      ...
      previewGearSetPieces(setId,count){
        let set=ACTIVE_SET_DEFINITIONS.find(entry=>entry.id===setId);if(!set)return null;
        count=Math.max(0,Math.min(SET_GEAR_SLOTS.length,Math.floor(Number(count)||0)));
        ...
        batchEquipmentChanges(()=>{for(const slot of GEAR_SLOTS)save.equipped[slot]=null;
          let progressionSlots=['hat','scarf','chest','boots','weapon'];
          for(let index=0;index<progressionSlots.length;index++){
            let slot=progressionSlots[index],item=ACTIVE_SET_ITEMS.find(entry=>entry.setId===setId&&entry.slot===slot),gear=rollGearInstance(item,Math.max(set.minLevel,save.level),1.08);
            save.gear.push(gear);if(index<count)save.equipped[slot]=gear.uid
          }
          if(count===SET_GEAR_SLOTS.length)for(const item of ACTIVE_SET_ITEMS.filter(entry=>entry.setId===setId&&entry.cosmeticOnly)){let gear=rollGearInstance(item,Math.max(set.minLevel,save.level),1.08);save.gear.push(gear);save.equipped[item.slot]=gear.uid}
        });
      ...
      spawnFoundationLegs(itemId,equipNow){
        return null
      },
      ...
      equipmentState(){
        return{slots:GEAR_SLOTS.slice(),visualSlots:VISUAL_GEAR_SLOTS.slice(),setSlots:SET_GEAR_SLOTS.slice(),anchors:Object.fromEntries(Object.entries(EQUIPMENT_SLOT_ANCHORS).map(([slot,anchor])=>[slot,Object.assign({},anchor)])),equipped:Object.assign({},save.equipped),selectedGearUid,gearFilter,gearRarityFilter,gearSort,previewing:false,unified:ui.gearPanel.classList.contains('adventureBagPanel')&&!ui.gearPanel.classList.contains('loadoutMode')}
      },
      ...
      addSpinTestLoot(amount){
        if(mode!=='run')startRun();let common=LOOT_BY_RARITY.common,pool=common&&common.length?common:LOOT_ITEMS,total=Math.max(0,Math.min(24,Math.floor(Number(amount)||0)));
      ...
      gearVisualCoverage(){
        ...
        return{items:LOOT_ITEMS.length,setItems:ACTIVE_SET_ITEMS.length,archivedItems:ARCHIVED_GEAR_ITEMS.length,sets:ACTIVE_SET_DEFINITIONS.length,profiles:0,missing,assets,atlasPaths:[...new Set(assets.map(entry=>entry.path).filter(Boolean))],usesProductionAssets:String(equipmentLayerTexture).includes('drawProductionGearAsset'),usesPieceGeometry:false,usesStripePattern:false,usesLegacyGearOverlay:/drawPappaGearBack|drawPappaGearFront/.test(String(drawPappaHammer)),modularChannels:EQUIPMENT_VISUAL_CHANNELS.map(channel=>channel.id),baseAsset:pappaHammerImage.src}
      },
      gearMaskBounds(pose,frame){
        if(!PAPER_DOLL_POSES.includes(pose)||frame<0||frame>7)return null;
        return Object.fromEntries(ASSET_GEAR_SLOTS.map(slot=>[slot,paperDollMaskFrame(pose,slot,frame).bounds]))
      }
    }
  }
