(function(){
  ...
  function drawMiniMap(){
    ...
    let rareCaches=caches.filter(cache=>cache.rare&&!cache.opened),pulse=.85+Math.sin(performance.now()/180)*.15;for(const cache of rareCaches){if(!inBounds(cache.x,cache.y))continue;let p=toMap(cache.x,cache.y),r=3.2*pulse;miniCtx.save();miniCtx.translate(p.x,p.y);miniCtx.rotate(Math.PI/4);miniCtx.fillStyle='#ffe19a';miniCtx.shadowColor='#f2c14f';miniCtx.shadowBlur=6;miniCtx.fillRect(-r/2,-r/2,r,r);miniCtx.restore()}if(bossLootChest&&!bossLootChest.cleanupLocked&&inBounds(bossLootChest.x,bossLootChest.y)){let p=toMap(bossLootChest.x,bossLootChest.y),r=(4.2+(bossLootChest.rank||0)*.4)*pulse;miniCtx.save();miniCtx.translate(p.x,p.y);miniCtx.fillStyle=bossLootChest.color;miniCtx.strokeStyle='#fff4d2';miniCtx.lineWidth=1.2;miniCtx.shadowColor=bossLootChest.color;miniCtx.shadowBlur=10;miniCtx.beginPath();miniCtx.arc(0,0,r,0,Math.PI*2);miniCtx.fill();miniCtx.stroke();miniCtx.globalAlpha=.7;miniCtx.beginPath();miniCtx.arc(0,0,r+2.2,0,Math.PI*2);miniCtx.stroke();miniCtx.restore()}
    ...
    let cacheCount=rareCaches.length+(bossLootChest&&!bossLootChest.cleanupLocked?1:0);ui.miniMapCacheCount.textContent=cacheCount?'\u2726 '+cacheCount:'';ui.miniMapCacheCount.classList.toggle('show',cacheCount>0)
  }
  function draw(){
