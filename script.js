(function(){
  ...
  function toggleBulkGear(uid){let gear=save.gear.find(entry=>entry.uid===uid);if(!gear)return;if(gearIsEquipped(gear)){notice('EQUIPPED GEAR IS PROTECTED','#79a67e');return}if(gearIsLocked(gear)){notice('LOCKED GEAR IS PROTECTED','#ffc928');return}if(gearBulkSelection.has(uid))gearBulkSelection.delete(uid);else gearBulkSelection.add(uid);selectedGearUid=uid;resetFilteredSaleArm();updateGearInventoryStates();renderGearSelection();renderGearBulkActions()}
  ...
  function renderGearDetail(gear,copies){
    ...
    let actions=document.createElement('div');actions.className='gearDetailActions';let equip=document.createElement('button');equip.className='equipGear';equip.textContent=equipped?'REMOVE':worn?'REPLACE':'EQUIP';equip.setAttribute('data-help-title',equipped?'Remove Gear':worn?'Replace Gear':'Equip Gear');equip.setAttribute('data-help',equipped?'Remove this item from Pappa Hammer without selling it.':'Wear this item now. It replaces the current '+GEAR_SLOT_META[item.slot].name.toLowerCase()+' and updates Pappa visually.');equip.addEventListener('click',()=>equipGear(gear.uid));let sell=document.createElement('button');sell.className='sellGear';sell.textContent=equipped?'SAFE':locked?'LOCKED':gearActionConfirming(gear.uid,'sell')?'CONFIRM SELL':'SELL  $'+gearUnitValue(gear);sell.setAttribute('data-help-title','Sell Gear');sell.setAttribute('data-help',equipped?'Equipped gear is protected and cannot be sold.':locked?'Locked gear cannot be sold.':'Sell this item permanently for $'+gearUnitValue(gear)+'.');sell.disabled=equipped||locked;sell.addEventListener('click',()=>sellGear(gear.uid));let salvage=document.createElement('button');salvage.className='salvageGear';salvage.textContent=equipped?'SAFE':locked?'LOCKED':gearActionConfirming(gear.uid,'salvage')?'CONFIRM SALVAGE':'SALVAGE  '+salvageRewardText(gear);salvage.setAttribute('data-help-title','Salvage Gear');salvage.setAttribute('data-help',equipped?'Equipped gear is protected and cannot be salvaged.':locked?'Locked gear cannot be salvaged.':'Destroy this item for '+salvageRewardText(gear)+'.');salvage.disabled=equipped||locked;salvage.addEventListener('click',()=>salvageGear(gear.uid));actions.append(equip,sell,salvage);root.appendChild(actions);ui.gearDetail.appendChild(root)
  }
  ...
  function activateGearEntry(event,gear){
    ...
    if(doubleTap){event.preventDefault();event.stopPropagation();gearTapUid=null;gearTapAt=0;gearQuickActionUntil=now+360;let item=gearDefinition(gear);if(!item||!GEAR_SLOTS.includes(item.slot))return;if(save.equipped[item.slot]===gear.uid)unequipGearSlot(item.slot);else equipGear(gear.uid);return}
    clearGearActionConfirm();selectedGearUid=gear.uid;gearTapUid=touch?gear.uid:null;gearTapAt=touch?now:0;paperDollPreview(gear);updateGearInventoryStates();renderGearSelection()
  }
  ...
  function renderGearInventoryArea(){
    ...
    for(const gear of items){
      let item=gearDefinition(gear),rarity=LOOT_RARITIES[item.rarity],equipped=gearIsEquipped(gear),locked=gearIsLocked(gear),selected=gear.uid===selectedGearUid,entry=document.createElement('button'),copyCount=copies[gear.itemId]||1;
      let bulkSelected=gearBulkSelection.has(gear.uid);entry.className='gearBagSlot rarity'+rarity.rank+(equipped?' equipped':'')+(locked?' locked':'')+(selected&&!gearBulkMode?' selected':'')+(gearBulkMode&&!equipped&&!locked?' bulkSelectable':'')+(bulkSelected?' bulkSelected':'');entry.style.setProperty('--gear-color',rarity.color);entry.style.setProperty('--gear-glow',rarity.glow);entry.setAttribute('data-item',gear.uid);entry.setAttribute('data-info-ready','');entry.setAttribute('role','option');entry.setAttribute('aria-selected',gearBulkMode?bulkSelected?'true':'false':selected?'true':'false');entry.draggable=!mobileArmory()&&!gearBulkMode;
  ...
  function returnBase(survived,status){
    ...
    if(survived){
      ...
      for(const slot of GEAR_SLOTS){if(save.equipped[slot])continue;let recovered=reportLoot.filter(gear=>gearDefinition(gear).slot===slot).sort(compareGearPriority);if(recovered[0])save.equipped[slot]=recovered[0].uid}
