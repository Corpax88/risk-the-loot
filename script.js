(function(){
  ...
  const blackHoleVfxAtlas=loadImage('assets/black-hole-hammer-vfx-v1.png');
  ...
  const HAMMERSTORM={acquireRadius:410,leapDuration:.18,pulseInterval:.115,baseRadius:108,maxLeap:315,minSpin:.16,vortexScale:1.38,vortexGap:72};
  ...
  const GEAR_SIGNATURES={
    ...
    blackHole:{role:'GRAVITY',name:'GRAVITY WELL',unlock:'Every fifth hammer strike forms a singularity that pulls the pack inward.',mastery:'Every third strike creates a wider, stronger vortex before the impact collapses.',color:'#4bbcff'}
  };
  ...
  function updateDashMotion(dt,stats){
    ...
    if(save.settings.particles&&player.dashFx<=0){player.dashFx+=mobileArmory()?.065:.035;spawnParticle({x:player.x-player.dashX*18,y:player.y-player.dashY*18,vx:-player.dashX*40,vy:-player.dashY*40,life:.22,max:.22,r:7,color:stats.stormDash?'#79e7f2':stats.ram?'#f2c14f':'#47c5b6'})}
  ...
  function tryHammerSpin(){
    ...
    player.spinPack=targets.length;player.spinPeakPack=targets.length;player.spinLeap=targets.length?HAMMERSTORM.leapDuration:.001;player.spinLeapMax=player.spinLeap;player.spinStartX=player.x;player.spinStartY=player.y;player.spinTargetX=landing.x;player.spinTargetY=landing.y;player.spinTime=0;player.spinPulse=0;player.spinAngle=0;player.spinShock=0;player.spinHits=0;player.spinKills=0;player.spinCoins=0;player.spinHeal=0;player.spinLifeTargets=new Set();player.spinVortexX=player.x;player.spinVortexY=player.y;player.inv=Math.max(player.inv,player.spinLeap+.08);player.fire=Math.max(player.fire,.2);effects.push({kind:'spinCharge',x:player.x,y:player.y,r:8,maxR:58+Math.min(44,targets.length*1.35),life:.26,max:.26,color:targets.length>=24?'#ffc928':'#d6aa58'});burst(player.x,player.y,targets.length>=24?'#ffc928':'#d6aa58',Math.min(28,12+Math.floor(targets.length/3)),1.05+Math.min(.55,targets.length*.012));shake=Math.max(shake,targets.length>=24?6:3);sound('spinStart');showHammerstormHelpOnce();return true
  }
  function updateHammerVortex(dt,stats){
    let radius=stats.spinRadius*HAMMERSTORM.vortexScale;player.spinVortexX=player.x;player.spinVortexY=player.y;
    for(const enemy of enemies){
      if(enemy.dead)continue;let dx=player.x-enemy.x,dy=player.y-enemy.y,distance=Math.hypot(dx,dy)||1,deadzone=player.r+enemy.r+HAMMERSTORM.vortexGap;if(distance>radius+enemy.r)continue;
      if(distance<=deadzone){enemy.knockVx=0;enemy.knockVy=0;enemy.knockTime=Math.max(enemy.knockTime,Math.min(.05,dt*2.5));continue}
      let gap=distance-deadzone,pull=Math.min(enemy.boss?82:245,(enemy.boss?28:72)+gap*(enemy.boss?.32:1.6)),scale=Math.min(1,dt*18);enemy.knockVx+=(dx/distance*pull-enemy.knockVx)*scale;enemy.knockVy+=(dy/distance*pull-enemy.knockVy)*scale;enemy.knockTime=Math.max(enemy.knockTime,Math.min(.065,dt*2.5))
    }
  }
  function hammerSpinPulse(stats){
    ...
    for(const enemy of pulseTargets){
      ...
      enemy.spinLaunched=!enemy.boss&&enemy.hp<=damage;damageEnemy(enemy,damage,enemy.x,enemy.y,true);hits++;if(critical){crits++;if(crits<=4)effects.push({kind:'spinCrit',x:enemy.x,y:enemy.y,r:5,maxR:38,life:.26,max:.26,color:'#f4ead6'})}if(!player.spinLifeTargets.has(enemy)){player.spinLifeTargets.add(enemy);newLifeTargets++}
    }
    ...
    let shockEvery=Math.max(3,6-Math.floor(Math.max(player.spinPack,hits)/15));if(player.spinShock%shockEvery===0)physicalShockwave(player.x,player.y,radius*1.48,shotDamage()*stats.damage*stats.spinDamage*.24,'#f0c66a',radius*.72,{mode:'none'});
  ...
  function updateHammerSpin(dt,stats,move){
    ...
    if(player.spinTime>0){player.spinTime+=dt;player.spinAngle+=dt*22*stats.spinRate;player.spinPulse-=dt;player.attackAnim=Math.max(player.attackAnim,.16);if(!updateDashMotion(dt,stats)){player.x+=move.x*player.speed*stats.speed*.34*dt;player.y+=move.y*player.speed*stats.speed*.34*dt;if(move.x||move.y){player.lastX=move.x;player.lastY=move.y}}updateHammerVortex(dt,stats);let livePack=nearbyEnemyCount(player.x,player.y,245,false);player.spinPeakPack=Math.max(player.spinPeakPack,livePack);player.spinPack=Math.max(player.spinPack,livePack);while(player.spinPulse<=0){hammerSpinPulse(cargoStats());player.spinPulse+=HAMMERSTORM.pulseInterval/stats.spinRate}if(!spinHeld&&player.spinTime>=HAMMERSTORM.minSpin)finishHammerSpin();return 'spin'}
    return ''
  }
  ...
  function triggerGravityWell(x,y,damage,tier){
    let mastered=tier===2,radius=mastered?188:154,hits=physicalShockwave(x,y,radius,damage*(mastered?.46:.3),'#4bbcff',0,{mode:'vortex',strength:mastered?300:220});
    effects.push({kind:'blackHoleVfx',x,y,r:8,maxR:radius,life:.76,max:.76,color:'#4bbcff',size:radius*2.12});
  ...
  function draw(){
    ...
    for(const fx of effects){
      ...
      else if(fx.kind==='blackHoleVfx'&&imageReady(blackHoleVfxAtlas)){let frame=Math.min(7,Math.floor(progress*8)),cellW=(blackHoleVfxAtlas.naturalWidth||blackHoleVfxAtlas.width)/4,cellH=(blackHoleVfxAtlas.naturalHeight||blackHoleVfxAtlas.height)/2,size=(fx.size||fx.maxR*2)*(0.78+progress*.22);ctx.globalCompositeOperation='screen';ctx.globalAlpha=Math.min(1,alpha*1.35);ctx.drawImage(blackHoleVfxAtlas,frame%4*cellW,Math.floor(frame/4)*cellH,cellW,cellH,-size/2,-size/2,size,size);ctx.globalCompositeOperation='source-over'}
