...
test('every production set fills and centers equipped slot previews',async({page})=>{
  ...
  for(const set of catalog){
    ...
    await expect.poll(()=>page.locator('#gearLoadoutSlots .equippedGearPreview:not([hidden])').count()).toBe(7);
    expectFilledAndCentered(await previewMetrics(page));
  }
});

test('Stormcaller chest, cape and boots remain large, clipped and sharp on desktop and iPhone',async({browser})=>{
  for(const setup of [
    ...
    for(const slot of ['cape','chest','boots'])expect(metrics.find(entry=>entry.slot===slot).maxRatio).toBeGreaterThan(.6);
    await page.locator('#gearCharacterStage').screenshot({
      path:path.join('test-results','equipped-slot-previews',`stormcaller-${setup.name}.png`),
      animations:'disabled'
    });
    await context.close();
  }
...
test('rapid replacement and reload leave one current preview per equipped display slot',async({page})=>{
  ...
  const loadouts=await page.evaluate(()=>{
    const bySlot={};
    for(const item of window.__riskTest.equipmentInventory())(bySlot[item.slot]||(bySlot[item.slot]=[])).push(item.uid);
    return [0,1].map(index=>Object.values(bySlot).map(items=>items[index]).filter(Boolean))
  });
