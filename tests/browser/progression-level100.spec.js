...
test('fresh and old-schema saves start cleanly at Level 1',async({page})=>{
  ...
  expect(state).toMatchObject({maxLevel:100,totalXpToMax:8986,saveVersion:12,progress:{level:1,current:0,required:10,capped:false}});
...
test('exact, multi-level and capped XP updates stay synchronized',async({page})=>{
  ...
  expect(state.progress).toMatchObject({level:99,current:203,required:204,capped:false});
  state=await page.evaluate(()=>window.__riskTest.grantPlayerXp(1));
