...
assert(html.includes('style.css?v=0.4.0')&&html.includes('script.js?v=0.4.0'),'release assets are not versioned');
assert(!/v0\.[23]\.0/.test(html+css+script),'stale release version found');
assert(script.includes('DEPTH_THRESHOLDS=[0,55,120,195,280]'),'v0.4 expedition pacing is missing');
assert(html.includes('id="resultTime"')&&html.includes('id="routeProgressFill"'),'playtest readout or route progress is missing');
