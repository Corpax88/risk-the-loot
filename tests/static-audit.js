...
assert.equal(progression.TOTAL_XP_TO_MAX,173388,'Level 100 total XP changed unexpectedly');
...
assert(script.includes('pappaHammerImage')&&script.includes('buildPappaHammerAtlas')&&script.includes('drawEquipmentCharacterFrame')&&script.includes("assets/pappa-hammer-player-mobile-v1.png?v=20260807-mobile-master-v1"),'approved mobile Pappa Hammer master is not wired');
assert(script.includes('EQUIPMENT_VISUAL_CHANNELS')&&script.includes('equipmentLayerTexture')&&!script.includes('pappaHammerMotionGuides')&&!script.includes("loadImage('assets/pappa-hammer-idle-v2.png')")&&!script.includes("loadImage('assets/pappa-hammer-run-v2.png')")&&!script.includes("loadImage('assets/pappa-hammer-attack-v2.png')"),'legacy body render sources are still wired');
assert(!script.includes('paperDollSetSprites')&&!script.includes('paperDollSetReady'),'legacy baked full-set character renderer is still wired');
assert(script.includes("assets/black-hole-vortex-spawn-v2.png")&&script.includes("assets/black-hole-vortex-idle-v2.png")&&script.includes("assets/black-hole-vortex-pull-v2.png")&&script.includes("assets/black-hole-vortex-pulse-v2.png")&&script.includes("assets/black-hole-vortex-dash-v2.png")&&script.includes("assets/black-hole-vortex-collapse-v2.png")&&script.includes("assets/black-hole-vortex-burst-v2.png"),'preserved Black Hole gameplay VFX are not wired');
...
assert(script.includes("assets/handlava-hit-splash-v1.png")&&script.includes("for(const phase of ['idle','extend','grab','swing','throw','retract'])")&&script.includes("loadImage('assets/handlava-'+phase+'-v1.png')"),'preserved Handlava gameplay VFX are not wired');
...
assert(script.includes("assets/ancient-ent-v1.png")&&script.includes("assets/nature-root-trap-v1.png"),'preserved Ancient Ent or Rootwhip gameplay VFX are not wired');
...
assert(css.includes('@keyframes pappaHammerFrames')&&css.includes('.gearCharacterHero')&&css.includes('background-image:none'),'live modular Gear Locker animation is missing');
...
assert(html.includes('id="miniMapCanvas"')&&html.includes('id="miniMapCacheCount"')&&script.includes('function drawMiniMap')&&script.includes('for(const cache of caches){if(cache.opened)continue;cacheCount++'),'all-cache minimap is missing');
...
assert(script.includes('SAVE_VERSION=16')&&script.includes('if(raw.version!==SAVE_VERSION)raw={}')&&script.includes('materials:0,legendaryCores:0')&&script.includes('gear:[]')&&script.includes('level:1,xp:0')&&script.includes('equipped:emptyEquipment()'),'mobile master development reset or progression model is missing');
...
assert(script.includes("const GEAR_SLOTS=['hat','cape','chest','legs','boots','scarf','weapon','necklace','ring1','ring2']")&&script.includes("const VISUAL_GEAR_SLOTS=['cape','legs','boots','chest','scarf','hat','weapon']")&&script.includes("id:'legs',sourceSlot:'legs'")&&script.includes('gearStats')&&script.includes('gearArtMarkup'),'canonical equipment slots, stats, or visual equipment are missing');
...
assert(html.includes('id="gearLockerButton"')&&html.includes('id="gearGrid"')&&html.includes('class="gearCharacterHero domLayeredFigure"')&&['cape','baseBody','legs','boots','chest','scarf','hat','weapon'].every(layer=>html.includes('data-character-layer="'+layer+'"'))&&!html.includes('data-character-layer="fullFigure"')&&script.includes('function refreshArmoryCharacterLayers'),'Gear Locker or canonical layered character preview is missing');
...
for(const retiredClass of ['windowGlow','pipeA','pipeB','forge','rig','rigHead','rigBody','rigArm','rigLeg','scrapHeap','floorLine','upgradeDeck','upgradeCard','workshopEvolution','evolutionA','evolutionB','riskPanel','riskChoice','riskChoices','riskReadout','extractChoice','pushChoice','lootResult','gearCard','gearCardIcon','gearCardTitle','gearCardMeta','gearCardActions','gearCharacterHeading','equippedProtection','gearSetName','gearBagReadout','gearDetailStats','gearInventoryFooter','gearDetailMeta','gearDetailLore','adventureBagHeader','adventureBagEmblem','gearCategoryRail','animeBagFlap','animeBagFooter','gearModeTabs','bagViewPanel','loadoutViewPanel','gearCategoryRarity','gearCategoryPanel','gearLoadoutInfo','gearLoadoutWorkspace','loadoutSectionLabel','gearLoadoutHeroColumn','gearCompareCard','gearCompareSet','gearRarityTag','bossLootOpenSlot','bossLootEquippedArt'])assert(!new RegExp('\\.'+retiredClass+'(?![A-Za-z0-9_-])').test(css),'obsolete CSS selector survived: '+retiredClass);
...
assert(script.includes("const ACTIVE_SET_DEFINITIONS=Object.freeze([stormcallerSet])")&&script.includes('const ACTIVE_SET_ITEMS=Object.freeze([...stormcallerCoreItems,...stormcallerCosmeticItems])')&&script.includes('const LOOT_ITEMS=ACTIVE_SET_ITEMS')&&script.includes('const ARCHIVED_GEAR_ITEMS=Object.freeze(')&&script.includes('PAPPA_V1_FOUNDATION'),'Pappa V1 Stormcaller proof is not isolated from the archived production catalog');
assert(!script.includes("loadImage('assets/set-gear-atlas.png')")&&!script.includes("loadImage('assets/set-gear-drops.png')")&&!css.includes("background-image:url('assets/set-gear-atlas.png')"),'striped set placeholder atlas is still wired into runtime');
for(const eagerAtlas of ['blackHoleGearAtlas','stormcallerGearAtlas','lavaGearAtlas','natureGearAtlas','gearItemAtlas','setGearAtlas','legendaryGearAtlas'])assert(!script.includes('const '+eagerAtlas+'='),'unused eager atlas load survived: '+eagerAtlas);
assert(script.includes('function invalidateLootManifest')&&script.includes('lootManifestCacheVersion===lootManifestVersion'),'run loot manifest is not cached between drops');
assert(script.includes('function compareGearPriority')&&script.includes('function compareGearLootPriority'),'gear priority sorting is duplicated instead of centralized');
assert(script.includes('const routeTickNodes=Array.from(ui.routeTicks.children)')&&!script.includes("let ticks=[...ui.routeTicks.children]"),'route HUD still allocates its fixed node list on every refresh');
assert(script.includes('function syncText(element,value)')&&script.includes('function syncStyle(element,property,value)'),'hot HUD DOM writes are not guarded');
...
assert(html.includes('id="bossLootOverlay"')&&html.includes('id="bossLootEquip"')&&html.includes('id="bossLootKeep"')&&html.includes('id="bossLootSalvage"')&&html.includes('id="materialCount"')&&html.includes('id="bossLootExtract"')&&html.includes('id="bossLootPush"')&&script.includes('function continueAfterBoss')&&script.includes('function spawnExtractionAmbush'),'post-boss loot decision and risk loop is missing');
